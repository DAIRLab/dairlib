...
1. We do not yet support lcm signal scope for Ubuntu 20.04. Before building dairlib, remove `data = ["@signal_scope"]` from `signalscope/BUILD.bazel`. You may build signal scope from source via [DAIRLab's fork](https://github.com/DAIRLab/signal-scope) if you wish.
...
- If you run into ram/cpu limits while building, you can cap the number of threads bazel will use by adding `build --jobs=8` to `.bazelrc` (you may replace `8` with however many threads you wish to allow)
