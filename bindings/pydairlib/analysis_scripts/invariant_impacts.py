...
def rabbit_main():
  ...
  # J = np.vstack((J_r, J_l))
  impulse_to_joint_space = M_inv @ J_r.T
  invariant_space = linalg.null_space(impulse_to_joint_space.T).T
  ...
  for i in range(t.shape[0]):
    vel[i] = state_traj.value(t[i])[-nv:, 0]
    cc_vel[i] = invariant_space @ state_traj.value(t[i])[-nv:, 0]
  ...
  print(invariant_space @ (v_pre - v_post))
