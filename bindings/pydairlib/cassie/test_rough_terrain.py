...
def main(magnitude):
    ...
    for idx, cmd in enumerate(fwd_radio_cmds):
        ...
        for i in range(num_rollouts):
            ...
            # env = make_vec_env(make_radio_swing_ft_env, visualize=False)
            s = env.reset()
            for step in range(num_steps_per_rollout):
                action = np.zeros(SWING_FOOT_ACTION_DIM + 3)
                action[0] = cmd
                s, r, d, info = env.step(action)
                # roll, pitch = calc_stability_metrics(s)
                roll = info["total_roll_error"]
                pitch = info["total_pitch_error"]
                if d:
                    s = env.reset()
                avg_roll += roll
                avg_pitch += pitch
