...
class PlotStyler():
    ...
    def __init__(self, figure=None, nrows=1, ncols=1, directory=None):
        ...
        plt.subplots_adjust(left=0.1, right=0.85, bottom=0.15, top=0.75)  #
        ...
        # self.fig.add_axes([0.1, 0.15, 0.85, 0.75])  # List is [left,
        # bottom, width, height]
    ...
    def save_fig(self, filename):
        self.fig.savefig(
            self.directory + filename, dpi=self.dpi,
            bbox_inches='tight'
            )
        return

    def add_legend(self, labels, loc=0, subplot_index=0, ncol=1):
        legend = self.axes[subplot_index].legend(
            labels, loc=loc, ncol=ncol, columnspacing=0.5, fontsize=10
        )
