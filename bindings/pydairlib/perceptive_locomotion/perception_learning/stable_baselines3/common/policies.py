...
from pydairlib.perceptive_locomotion.perception_learning.stable_baselines3.common.distributions import (
...
from pydairlib.perceptive_locomotion.perception_learning.stable_baselines3.common.preprocessing import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
...
from pydairlib.perceptive_locomotion.perception_learning.stable_baselines3.common.type_aliases import PyTorchObs, Schedule
from pydairlib.perceptive_locomotion.perception_learning.stable_baselines3.common.utils import get_device, is_vectorized_observation, obs_as_tensor
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, rpo=False) -> Distribution:
        """
        ...
        #self.log_std = nn.Parameter(th.tensor([-3.6, -3.6, -3.9,], device='cuda:0'))
    ...
    def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor, RPO: bool = False) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        ...
        if self.share_features_extractor:
            ...
            latent_pi = self.mlp_extractor.forward_actor(pi_features)
            latent_vf = self.mlp_extractor.forward_critic(vf_features)
        distribution = self._get_action_dist_from_latent(latent_pi, rpo = RPO)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
