...
def _run_training(config, args):
    ...
    if args.test:
        ...
        model_path = 'torque1.zip' # x/logs2/rl_model_1728000_steps
        ...
        model = RecurrentPPO.load(model_path, env, learning_rate = linear_schedule(5e-6), max_grad_norm = 0.5, # linear_schedule(3e-6)
                        ...
                        batch_size=128, seed=1952, init_cnn_weights=False, # init_cnn_weights: Initialize critic CNN with Actor CNN
                        tensorboard_log=tensorboard_log)
...
def _main():
    ...
    config = {
        ...
        "total_timesteps": 300e6 if not args.test else 5000,
