...
def _multi_tensor_adadelta(
    ...
    *,
    ...
    for (
        ...
        for i in range(len(device_square_avgs)):
            device_square_avgs[i] = device_square_avgs[i].to(device_grads[i].device)
        ...
        std = tuple(std_tensor.to(d.device) for std_tensor, d in zip(std, deltas))
        torch._foreach_div_(deltas, std)
        
        deltas = tuple(delta.to(device_grad.device) for delta, device_grad in zip(deltas, device_grads))
        torch._foreach_mul_(deltas, device_grads)

        torch._foreach_mul_(device_acc_deltas, rho)
        
        for i in range(len(device_acc_deltas)):
            device_acc_deltas[i] = device_acc_deltas[i].to(deltas[i].device)
        ...
        if capturable and isinstance(lr, torch.Tensor):
            ...
            torch._foreach_add_(device_params, deltas)
       	    ...
       	    for i in range(len(device_params)):
       	        device_params[i] = device_params[i].to(deltas[i].device)
