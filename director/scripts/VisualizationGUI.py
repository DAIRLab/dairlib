...
class VisualizationGui(QWidget):
  ...
  def readJSONFile(self):
    ...
    if (self.plant == None):
      ...
      self.plant.Finalize()
      # self.position_idx_map = MakeNameToPositionsMap(self.plant)
      # self.velocity_idx_map = MakeNameToVelocitiesMap(self.plant)
      # self.effort_idx_map = MakeNameToActuatorsMap(self.plant)
    ...
    # self.state_recevier = RobotOutputReceiver(self.plant)

    # add more LCM subscriptions depending on the number of "lcm" data
    for name in self.lcmObjects:
      lcmMessage = self.lcmObjects[name]
      subscriber = lcmUtils.addSubscriber(lcmMessage.channel,
                                          messageClass=eval(lcmMessage.type),
                                          callback=lambda msg,
                                                          lcmMessage=lcmMessage,
                                                          name=name: self.abstract_handler(
                                            msg, name, lcmMessage))
  ...
  def placeCheckBoxes(self):
    ...
    for name in self.shapes:
      ...
      if (name not in self.checkBoxes):
        ...
        if (self.shapes[name].type == "point" or self.shapes[
          name].type == "line"):
          color = self.shapes[name].color
          self.checkBoxes[name].setStyleSheet(
            "color: rgb(" + str(color[0] * 255) + ", " + str(
              color[1] * 255) + ", " + str(color[2] * 255) + ")")
  ...
  def abstract_handler(self, msg, name, lcmMessage):
    ...
    if ("%d" not in field):
      ...
      curr_shape = self.shapes[name]
      x = lcmMessage.x

      # special case of an axis data
      if (curr_shape.type == "axes"):
      ...
      print(
        lcmMessage.index_element + "is not present in " + lcmMessage.index_field + " and so it cannot be visualized")
      print("")
  ...
  def handle_checkBox(self, name):
    ...
    if (self.checkBoxes[name].isChecked() == True):
      if (self.shapes[name].object != None and self.shapes[
        name].object.getProperty('Visible') == False):
        self.shapes[name].object.setProperty('Visible', True)

    else:
      if (self.shapes[name].object != None and self.shapes[
        name].object.getProperty('Visible') == True):
  ...
  def state_handler(self, msg):
    ...
    msg: the returning LCM message
    '''
    # start listening to messages once the JSON file has been read
    if (self.ready == True):
      ...
      for name in self.shapes:
        ...
        if (currShape.category == "kinematic"):
          ...
          next_loc = self.plant.CalcCenterOfMassPositionInWorld(
  ...
  def drawShape(self, curr_shape, next_loc, msg, rotation_matrix=None):
    '''
    ...
    if (curr_shape.type == "line"):
      # check if the duration has been initialized
      if (curr_shape.duration == None or len(curr_shape.points) == 0):
        curr_shape.duration = msg.utime / 1000000

      # visualize and trace line for 'history' seconds, adding points at a distance at least 10e-5
      if (((
               msg.utime / 1000000) - curr_shape.duration <= curr_shape.history) or curr_shape.history <= 0):
        # make sure to add at least 2 points before starting to check for the distance between points
        if (len(curr_shape.points) < 2):
          curr_shape.points.append(next_loc)
          d = DebugData()
          d.addPolyLine(curr_shape.points, radius=curr_shape.thickness,
                        color=curr_shape.color)
          self.meshcat.SetLine(curr_shape.name, np.array(curr_shape.points).T,
                               line_width=curr_shape.thickness * 1000,
                               rgba=Rgba(curr_shape.color[0],
                                         curr_shape.color[1],
                                         curr_shape.color[2],
                                         1.0))

          if (curr_shape.object == None):
            curr_shape.object = vis.showPolyData(d.getPolyData(),
                                                 curr_shape.name)
            curr_shape.object.setProperty('Color', curr_shape.color)
          else:
            curr_shape.object.setPolyData(d.getPolyData())

        else:
          if (np.linalg.norm(
              np.array(next_loc) - np.array(curr_shape.points[-1])) >= 10e-5):
            curr_shape.points.append(next_loc)
            d = DebugData()
            d.addPolyLine(curr_shape.points, radius=curr_shape.thickness,
                          color=curr_shape.color)
      ...
      elif (curr_shape.history > 0):
        if (len(curr_shape.points) == 0):
          curr_shape.duration = msg.utime / 1000000
        else:
          # visualize and trace line for 'history' seconds, adding points at a distance at least 10e-5
          if (np.linalg.norm(
              np.array(next_loc) - np.array(curr_shape.points[-1])) >= 10e-5):
            curr_shape.points.popleft()
            curr_shape.points.append(next_loc)
            d = DebugData()
            d.addPolyLine(curr_shape.points, radius=curr_shape.thickness,
                          color=curr_shape.color)
    ...
    # draw a point
    elif (curr_shape.type == "point"):
      d = DebugData()
      d.addSphere(next_loc, radius=curr_shape.radius)
      # create a new point
      if (curr_shape.created == True):
        curr_shape.object = vis.showPolyData(d.getPolyData(), curr_shape.name)
        # set color and transparency of point
        curr_shape.object.setProperty('Color', curr_shape.color)
        curr_shape.object.setProperty('Alpha', curr_shape.alpha)
        curr_shape.created = False
      else:
        # update the location of the last point
        curr_shape.object.setPolyData(d.getPolyData())

    # draw a set of axes
    elif (curr_shape.type == "axes"):
      # get the rotation matrix
      rot_matrix = None
      if (curr_shape.category != "lcm"):
        rigTrans = self.plant.EvalBodyPoseInWorld(self.context,
      ...
      d = DebugData()
      for i in range(3):
        d.addArrow(next_loc, next_loc + (rot_matrix[i] * curr_shape.length),
                   headRadius=0.03, color=colors[i])
