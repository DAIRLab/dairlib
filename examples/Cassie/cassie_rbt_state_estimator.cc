...
EventStatus CassieRbtStateEstimator::Update(
  ...
  // Returning if u is zero (measurement error)
  if (u_curr.isZero()) {
  ...
  if (current_time > prev_t) {
    ...
    // std::cout << ekf_bias_predicted << std::endl;

    // std::cout << contacts_curr_->at(0) << " " << contacts_curr_->at(1) << " "
    //          << contacts_curr_->at(2) << " " << contacts_curr_->at(3)
    //          << std::endl;
