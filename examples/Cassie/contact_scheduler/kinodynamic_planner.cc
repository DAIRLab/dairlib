...
void KinodynamicPlanner::AddMode(const LinearSrbdDynamics& dynamics,
                                 BipedStance stance, const MatrixXd& reset,
                                 int N) {
  ...
  pp.push_back(
      prog_.NewContinuousVariables(kLinearDim_, "p_" + std::to_string(stance)));
  total_knots_ += N;
  modes_.push_back(mode);
  nmodes_++;
}

void KinodynamicPlanner::FinalizeModeSequence() {
  xx.push_back(prog_.NewContinuousVariables(nx_, "x_f"));
  uu.push_back(prog_.NewContinuousVariables(nu_, "u_f"));
  if (use_residuals_) {
    residual_manager_ = std::make_unique<systems::MpcPeriodicResidualManager>(
        total_knots_ + 1, modes_.front().dynamics.A, modes_.front().dynamics.B,
        modes_.front().dynamics.b);
  }
}

void KinodynamicPlanner::SetReachabilityBoundingBox(
...
void KinodynamicPlanner::CheckProblemDefinition() {
...
void KinodynamicPlanner::MakeTerrainConstraints(const Vector3d& normal,
  ...
  for (int i = 0; i < nmodes_; i++) {
    terrain_angle_.push_back(
        prog_.AddLinearEqualityConstraint(TerrainNormal, TerrainPoint, pp.at(i))
            .evaluator()
            .get());
  }
}

void KinodynamicPlanner::MakeDynamicsConstraints() {
  for (int j = 0; j < nmodes_; j++) {
    ...
    for (int i = 0; i < mode.N; i++) {
      ...
      dynamics_.push_back(prog_.AddLinearEqualityConstraint(
          Aeq, beq,
          {xx.at(j * mode.N + i), xx.at(j * mode.N + i + 1),
           uu.at(j * mode.N + i), uu.at(j * mode.N + i + 1),
           pp.at(mode.stance)}));
    }
  }
...
void KinodynamicPlanner::UpdateKinematicConstraints(int n_until_stance,
                                                    int fsm_state) const {
  ...
  for (int i = n_until_stance; i <= n_until_stance + next_mode.N; i++) {
    kinematic_constraint_.push_back(prog_.AddLinearConstraint(
        A, nominal_foot_pos_.at(next_mode.stance) - kin_bounds_,
  ...
  for (int i = n_until_stance + next_mode.N; i <= total_knots_; i++) {
    kinematic_constraint_.push_back(prog_.AddLinearConstraint(
        A, nominal_foot_pos_.at(curr_mode.stance) - kin_bounds_,
...
void KinodynamicPlanner::UpdateDynamicsConstraints(const Eigen::VectorXd& x,
                                                   int n_until_next_stance,
                                                   int fsm_state) const {
  ...
  if (n_until_next_stance == mode.N) {
    ...
    dynamics_.at(idx + mode.N) = prog_.AddLinearEqualityConstraint(
        Aeq2, beq2,
        {xx.at(idx + mode.N), xx.at(idx + mode.N + 1), uu.at(idx + mode.N),
         uu.at(idx + mode.N + 1), pp.at(fsm_state)});
  }
  std::cout << "\n";
}

// void KinodynamicPlanner::UpdateResidualDynamicsConstraints(const
// Eigen::VectorXd& x,
//                                         int n_until_next_stance, int
//                                         fsm_state) const {
//
//  auto& mode = modes_.at(fsm_state);
//  Vector3d pos = plant_.CalcFootPosition(x, mode.stance);
...
//          residual_dynamics{mode.dynamics.A, mode.dynamics.B,
//          mode.dynamics.b},
...
//    Aeq = MatrixXd::Zero(nx_, 2*(nx_ + nu_) + kLinearDim_);
//    beq = VectorXd::Zero(nx_);
//    LinearSrbdDynamics dyn_f = {A_matrix_t::Zero(), B_matrix_t::Zero(),
//    b_vector_t::Zero()}; residual_manager_->AddResidualToDynamics(
...
void KinodynamicPlanner::UpdateFootPlacementCost(
    int fsm_state, const Eigen::VectorXd& up_next_foot_target,
  ...
  foot_target_cost_.at(1 - fsm_state)
      ->UpdateCoefficients(2.0 * Wp_, -2.0 * Wp_ * up_next_foot_target);
}

void KinodynamicPlanner::MakeInitialStateConstraint() {
  initial_state_ =
      prog_
          .AddLinearEqualityConstraint(MatrixXd::Identity(nx_, nx_),
                                       VectorXd::Zero(nx_), xx.at(0))
          .evaluator()
          .get();
}

void KinodynamicPlanner::MakeKinematicReachabilityConstraints() {
  MatrixXd A = MatrixXd::Zero(kLinearDim_, 2 * kLinearDim_);
  A << Matrix3d::Identity(), -Matrix3d::Identity();
  for (int j = 0; j < nmodes_; j++) {
    auto mode = modes_.at(j);
    for (int i = 0; i <= mode.N; i++) {
      kinematic_constraint_.push_back(prog_.AddLinearConstraint(
          A, nominal_foot_pos_.at(mode.stance) - kin_bounds_,
...
void KinodynamicPlanner::MakeCost() {
  ...
  for (int i = 0; i <= total_knots_; i++) {
    ...
    tracking_cost_.push_back(
        prog_.AddQuadraticErrorCost(Q, x_des_, xx.at(i)).evaluator().get());
    input_cost_.push_back(
        prog_.AddQuadraticErrorCost(R, unom, uu.at(i)).evaluator().get());
  }
  for (int i = 0; i < nmodes_; i++) {
    foot_target_cost_.push_back(
        prog_.AddQuadraticErrorCost(Wp_, nominal_foot_pos_.at(i), pp.at(i))
            .evaluator()
            .get());
  }
}

void KinodynamicPlanner::AddTrackingObjective(const VectorXd& xdes,
                                              const MatrixXd& Q) {
  Q_ = Q;
  x_des_ = xdes;
}

void KinodynamicPlanner::SetTerminalCost(const MatrixXd& Qf) { Qf_ = Qf; }

void KinodynamicPlanner::AddInputRegularization(const Eigen::MatrixXd& R) {
  R_ = R;
}

void KinodynamicPlanner::AddFootPlacementRegularization(
    const Eigen::MatrixXd& W) {
  Wp_ = W;
}

void KinodynamicPlanner::GetMostRecentMotionPlan(
    const drake::systems::Context<double>& context,
    dairlib::lcmt_saved_traj* traj_msg) const {
  *traj_msg = most_recent_sol_;
}

EventStatus KinodynamicPlanner::DiscreteVariableUpdate(
    const drake::systems::Context<double>& context,
  ...
  if (use_fsm_) {
    ...
    if (fsm_state(0) != current_fsm_state(0)) {
      ...
      discrete_state->get_mutable_vector(prev_event_time_idx_)
              .get_mutable_value()
          << timestamp;
    }
  }
...
EventStatus KinodynamicPlanner::PeriodicUpdate(
    const drake::systems::Context<double>& context,
    drake::systems::DiscreteValues<double>* discrete_state) const {
  const OutputVector<double>* robot_output =
      (OutputVector<double>*)this->EvalVectorInput(context, state_port_);
  ...
  if (use_fsm_) {
    fsm_state = (int)(discrete_state->get_vector(current_fsm_state_idx_)
                          .get_value()(0));
    double last_event_time =
        discrete_state->get_vector(prev_event_time_idx_).get_value()(0);

    time_since_last_event =
        (last_event_time <= 0) ? timestamp : timestamp - last_event_time;

    if (use_residuals_) {
      residual_dynamics res =
          this->EvalAbstractInput(context, srbd_residual_port_)
    ...
    UpdateConstraints(plant_.CalcSRBStateFromPlantState(x), fsm_state,
                      time_since_last_event);
  } else {
  ...
  UpdateFootPlacementCost(fsm_state, Vector3d::Zero(), Vector3d::Zero());
  //
  ...
  most_recent_sol_ = MakeLcmTrajFromSol(result_, timestamp,
                                        time_since_last_event, x, fsm_state);
  return EventStatus::Succeeded();
}

void KinodynamicPlanner::UpdateTrackingObjective(const VectorXd& xdes) const {
  x_des_ = xdes;
...
void KinodynamicPlanner::UpdateConstraints(
  ...
  std::cout << "Time since last switch: " << std::to_string(t_since_last_switch)
            << std::endl;
  int n_until_next_state =
      modes_.at(fsm_state).N - std::floor(t_since_last_switch / dt_);
  if (use_residuals_) {
...
void KinodynamicPlanner::SetInitialGuess(
...
lcmt_saved_traj KinodynamicPlanner::MakeLcmTrajFromSol(
    const drake::solvers::MathematicalProgramResult& result, double time,
    double time_since_last_touchdown, const VectorXd& state,
    int fsm_state) const {
  ...
  for (int i = 0; i < total_knots_; i++) {
    ...
    orientation.block(0, i, 2 * kAngularDim_, 1)
        << xi.segment(kLinearDim_, kAngularDim_),
        xi.tail(kAngularDim_);
...
void KinodynamicPlanner::CopyDiscreteDynamicsConstraint(
    const SrbdMode& mode, bool current_stance, const Vector3d& foot_pos,
  ...
  if (!current_stance) {
    ...
    *b = -(mode.dynamics.A.block(0, nx_, nx_, kLinearDim_) * foot_pos +
           mode.dynamics.b);
  }
}

void KinodynamicPlanner::CopyCollocationDynamicsConstraint(
    const LinearSrbdDynamics& dyn, bool current_stance,
    const Eigen::Vector3d& foot_pos, const drake::EigenPtr<Eigen::MatrixXd>& A,
  ...
  double h = dt_;
  A->block(0, 0, nx_, nx_) = (0.125 * h) * Ax * Ax - 0.75 * Ax -
                             (1.5 / h) * MatrixXd::Identity(nx_, nx_);
  A->block(0, nx_, nx_, nx_) = -(0.125 * h) * Ax * Ax - 0.75 * Ax +
                               (1.5 / h) * MatrixXd::Identity(nx_, nx_);
