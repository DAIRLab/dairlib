...
CassieOutputReceiver::CassieOutputReceiver() {
  ...
  this->DeclareAbstractOutputPort("cassie_out_t",
                                  &CassieOutputReceiver::CopyOutput);
}

void CassieOutputReceiver::CopyOutput(const Context<double>& context,
                                      cassie_out_t* cassie_out) const {
  const auto& message = this->EvalAbstractInput(context, 0)
                            ->get_value<dairlib::lcmt_cassie_out>();
