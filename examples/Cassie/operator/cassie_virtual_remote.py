...
class KeyboardManager():
    ...
    def switch_motion_key(self, key):
        switcher = {
            K_w: np.array([self.delta_vx, 0.0, 0.0]),
            K_s: np.array([-self.delta_vx, 0.0, 0.0]),
            K_a: np.array([0.0, self.delta_vy, 0.0]),
            K_d: np.array([0.0, -self.delta_vy, 0.0]),
            K_RIGHT: np.array([0.0, 0.0, 0.1]),
            K_LEFT: np.array([0.0, 0.0, -0.1]),
        }
        return switcher.get(key, np.array([0, 0, 0]))
...
def main():
    ...
    while True:
        ...
        display_text3 = 'Yaw Vel: ' + '%.2f'%(keyboard.vel[2])
        text1 = fnt.render(display_text1, True, white, cassie_blue)
        text2 = fnt.render(display_text2, True, white, cassie_blue)
        text3 = fnt.render(display_text3, True, white, cassie_blue)
        text_rect1 = text1.get_rect()
        text_rect2 = text2.get_rect()
        text_rect3 = text3.get_rect()
        text_rect1.center = (250, 150)
        text_rect2.center = (250, 250)
        text_rect3.center = (250, 350)

        keyboard.screen.fill(cassie_blue)
        keyboard.screen.blit(text1, text_rect1)
        keyboard.screen.blit(text2, text_rect2)
        keyboard.screen.blit(text3, text_rect3)
        ...
        cassie_out_msg.pelvis.radio.channel[3] = keyboard.vel[2]
