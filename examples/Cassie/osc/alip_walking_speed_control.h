...
class AlipWalkingSpeedControl : public drake::systems::LeafSystem<double> {
 public:
  AlipWalkingSpeedControl(const drake::multibody::MultibodyPlant<double>& plant,
  ...
  const drake::systems::InputPort<double>& get_input_port_L() const {
    return this->get_input_port(L_port_);
  }
