...
DeviationFromCapturePoint::DeviationFromCapturePoint(
  ...
  heading_angle_port_ = this->DeclareVectorInputPort(BasicVector<double>(
      2)).get_index();
  //  transverse_vel_port_ = this->DeclareVectorInputPort(BasicVector<double>(
  //      2)).get_index();
...
void DeviationFromCapturePoint::CalcFootPlacement(const Context<double>& context,
  ...
  const BasicVector<double>* desired_heading = (BasicVector<double>*)
      this->EvalVectorInput(context, heading_angle_port_);
  //  const BasicVector<double>* transverse_velocity = (BasicVector<double>*)
  //      this->EvalVectorInput(context, transverse_vel_port_);
  VectorXd q = robot_output->GetPositions();
  VectorXd v = robot_output->GetVelocities();
  VectorXd desired_vel = desired_heading->get_value();
  ...
  //                                          global_com_pos_to_target_pos,
  //                                          params_of_no_turning_);

  // Calculate the current-desired yaw angle difference
  double heading_error = desired_vel(0) - approx_pelvis_yaw;
  ...
  if (!pause_walking_position_control) {
    ...
    double des_sagital_vel = desired_vel(1);
    ...
    double des_lateral_vel =
        ...
        //      desired_vel(0)*sin(desired_vel(0)) +
