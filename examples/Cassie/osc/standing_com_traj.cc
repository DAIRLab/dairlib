...
StandingComTraj::StandingComTraj(const RigidBodyTree<double>& tree,
                                 int pelvis_idx, int left_foot_idx,
                                 int right_foot_idx, double height)
    : tree_(tree),
...
void StandingComTraj::CalcDesiredTraj(
  ...
  Vector3d left_front =
      tree_.transformPoints(cache, front_contact_disp_, left_foot_idx_, 0);
  Vector3d left_rear =
      tree_.transformPoints(cache, rear_contact_disp_, left_foot_idx_, 0);
  Vector3d right_front =
      tree_.transformPoints(cache, front_contact_disp_, right_foot_idx_, 0);
  Vector3d right_rear =
      tree_.transformPoints(cache, rear_contact_disp_, right_foot_idx_, 0);
  Vector3d feet_center =
      (left_front + left_rear + right_front + right_rear) / 4;
