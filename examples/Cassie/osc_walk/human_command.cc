...
HumanCommand::HumanCommand(const RigidBodyTree<double>& tree, int pelvis_idx,
  ...
  // Discrete state which stores the desired yaw angle
  des_yaw_idx_ = DeclareDiscreteState(VectorXd::Zero(1));
...
EventStatus HumanCommand::DiscreteVariableUpdate(
  ...
  if (current_time > prev_time(0)) {
    ...
    // Get desried heading angle of pelvis
    ...
    discrete_state->get_mutable_vector(des_yaw_idx_).get_mutable_value()
        << desired_filtered_yaw;
...
void HumanCommand::CopyHeadingAngle(const Context<double>& context,
  ...
  double desried_heading_pos =
      context.get_discrete_state(des_yaw_idx_).get_value()(0);
