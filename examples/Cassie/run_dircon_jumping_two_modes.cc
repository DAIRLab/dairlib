...
void DoMain() {
  ...
  vector<double> max_dt;  // min_duration = knot_points * 0.03 ~= .3s
  min_dt.push_back(.01);
  min_dt.push_back(.01);
  max_dt.push_back(.1);
  max_dt.push_back(.1);
  ...
  if (!FLAGS_load_filename.empty()) {
    ...
    trajopt->SetInitialGuessForAllVariables(
        0.1 * VectorXd::Random(trajopt->decision_variables().size()));
    ...
    for (int i = 0; i < mode_lengths[1]; ++i) {
      int knot_point = mode_lengths[1] - 1 + i;
  ...
  // Printing values of the decision variables
  ...
  for (int mode = 0; mode < num_modes; ++mode) {
    ...
    const vector<string>& datatypes_wo_inputs =
        multibody::createStateAndActuatorNameVectorFromMap(
            pos_map, vel_map, std::map<string, int>());
...
void setKinematicConstraints(HybridDircon<double>* trajopt,
  ...
  trajopt->AddBoundingBoxConstraint(0.2, 0.2, xf(pos_map.at("base_x")));
  ...
  for (const auto& joint : joint_names) {
    trajopt->AddConstraintToAllKnotPoints(
        x(pos_map.at(joint)) <=
        plant.GetJointByName(joint).position_upper_limits()(0));
    trajopt->AddConstraintToAllKnotPoints(
        x(pos_map.at(joint)) >=
        plant.GetJointByName(joint).position_lower_limits()(0));
  }
  ...
  vector<int> stance_modes{0};
  // ALL BUT THE LAST 3 SEGMENTS (to ensure the feet can leave the ground
  for (int index = 0; index < (mode_lengths[0] - 3); index++) {
  ...
  const MatrixXd R = 0.05 * MatrixXd::Identity(n_u, n_u);
  trajopt->AddRunningCost(x.tail(n_v).transpose() * Q * x.tail(n_v));
  trajopt->AddRunningCost(u.transpose() * R * u);

  // Add some cost to hip roll and yaw
  double w_q_hip_roll = 0.0001;
  double w_q_hip_yaw = 0.0001;
  double w_q_hip_pitch = 0.0001;
  double w_q_quat_xyz = 0.0001;
  if (w_q_hip_roll != 0) {
    for (int i = 0; i < N; i++) {
...
vector<VectorXd> GetInitGuessForQStance(int num_knot_points,
  ...
  for (int i = 0; i < num_knot_points; i++) {
    ...
    Vector3d front_contact_disp;
    front_contact_disp << -0.0457, 0.112, 0;
    Vector3d rear_contact_disp;
    rear_contact_disp << 0.088, 0, 0;
    Vector3d left_toe_pos(0.02115, 0.12, 0.00);
    Vector3d right_toe_pos(0.02115, -0.12, 0.00);
    Vector3d left_heel_pos(-0.02115, 0.12, 0.00);
    Vector3d right_heel_pos(-0.02115, -0.12, 0.00);
...
vector<VectorXd> GetInitGuessForQFlight(int num_knot_points, double apex_height,
  ...
  q_ik_guess << quat.normalized(), 0.000889849, 0.000626865, rest_height,
      -0.0112109, 0.00927845, -0.000600725, -0.000895805, 1.15086, 0.610808,
      -1.38608, -1.35926, 0.806192, 1.00716, -M_PI / 2, -M_PI / 2;
  ...
  for (int i = 0; i < num_knot_points; i++) {
    ...
    Vector3d left_toe_pos(0.02115, 0.12, 0.00 + height_offset * 0.5);
    Vector3d right_toe_pos(0.02115, -0.12, 0.00 + height_offset * 0.5);
    Vector3d left_heel_pos(-0.02115, 0.12, 0.00 + height_offset * 0.5);
    Vector3d right_heel_pos(-0.02115, -0.12, 0.00 + height_offset * 0.5);
