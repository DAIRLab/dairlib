...
int do_main(int argc, char* argv[]) {
  ...
  if (FLAGS_floating_base) {
    CassieFixedPointSolver(plant, FLAGS_height, mu_fp, min_normal_fp, true,
                           toe_spread, &q, &u, &lambda);
  } else {
  ...
  auto left_toe_evaluator = multibody::WorldPointEvaluator(
      *plant_ad, left_toe.first, left_toe.second, Eigen::Matrix3d::Identity(),
      Eigen::Vector3d::Zero(), {1, 2});

  auto right_toe = RightToeFront(*plant_ad);
  auto right_toe_evaluator = multibody::WorldPointEvaluator(
      *plant_ad, right_toe.first, right_toe.second, Eigen::Matrix3d::Identity(),
      Eigen::Vector3d::Zero(), {1, 2});

  auto left_heel = LeftToeRear(*plant_ad);
  auto left_heel_evaluator = multibody::WorldPointEvaluator(
      *plant_ad, left_heel.first, left_heel.second);

  auto right_heel = RightToeRear(*plant_ad);
  auto right_heel_evaluator = multibody::WorldPointEvaluator(
      *plant_ad, right_heel.first, right_heel.second);
  ...
  AutoDiffVecXd x_ad =
      xul_ad.head(plant.num_positions() + plant.num_velocities());
  AutoDiffVecXd u_ad = xul_ad.segment(
  ...
  Q << FLAGS_Q_scale * 10 *
      ...
      FLAGS_Q_scale * 1 *
          Eigen::MatrixXd::Identity(plant.num_velocities(),
