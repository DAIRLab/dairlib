...
DEFINE_string(gains_filename, "examples/Cassie/osc/mptc_standing_gains.yaml",
...
int DoMain(int argc, char* argv[]) {
  ...
  plant_wo_springs.Finalize();
   ...
   std::unique_ptr<drake::multibody::MultibodyPlant<AutoDiffXd>> plant_ad =
            drake::systems::System<double>::ToAutoDiffXd(plant_wo_springs);
...
//                                      plant_w_springs, plant_wo_springs);
  TransTaskSpaceTrackingData center_of_mass_traj("com_traj", K_p_com, K_d_com,
                                      W_com * FLAGS_cost_weight_multiplier,
                                      plant_w_springs, plant_wo_springs, *plant_ad);
  ...
  builder.Connect(osc->get_mptc_output_port(),
