...
DEFINE_bool(xprod, false, "whether to use cross product basis");

// Code-gen of cross product basis
void xprod_basis_aff(const double in1[12], double basis[16])
{
  //     This function was generated by the Symbolic Math Toolbox version 8.7.
  //     10-May-2021 21:02:26
  std::copy(&in1[0], &in1[12], &basis[0]);
  basis[12] = in1[6] * in1[11] - in1[7] * in1[10];
  basis[13] = in1[8] * in1[11] - in1[9] * in1[10];
  basis[14] = in1[0] * in1[11] - in1[1] * in1[10];
  basis[15] = in1[3] * in1[11] - in1[4] * in1[10];
}
...
VectorXd xprod_basis (const VectorXd& x) {
  double out[16];
  xprod_basis_aff(x.data(), out);
  return Eigen::Matrix<double,16,1>(out);
}
...
int DoMain(int argc, char* argv[]) {
  ...
  string folder_base = FindResourceOrThrow(FLAGS_model_folder);

  int nz = FLAGS_xprod ? 16 : 12;
  int nu = 6;
  MatrixXd Al = MatrixXd::Zero(nz, nz);
  MatrixXd Bl = MatrixXd::Zero(nz, nu);
  MatrixXd bl = MatrixXd::Zero(nz,1);
  MatrixXd Ar = MatrixXd::Zero(nz, nz);
  MatrixXd Br = MatrixXd::Zero(nz, nu);
  MatrixXd br = MatrixXd::Zero(nz,1);

  KoopmanMPC::LoadDiscreteDynamicsFromFolder(folder_base, dt, &Al, &Bl, &bl, &Ar, &Br, &br);

  KoopmanDynamics left_stance_dynamics = {(FLAGS_xprod ? & xprod_basis : &poly_basis_1), Al, Bl, bl};
  KoopmanDynamics right_stance_dynamics = {(FLAGS_xprod ? & xprod_basis : &poly_basis_1), Ar, Br, br};
