...
class ContactEstimationTest : public ::testing::Test {
 protected:
  ...
  /*    : plant_(drake::multibody::MultibodyPlant<double>(1e-3)) {
    ...
    auto left_loop = LeftLoopClosureEvaluator(plant_);
    auto right_loop = RightLoopClosureEvaluator(plant_);
    fourbar_evaluator_->add_evaluator(&left_loop);
    fourbar_evaluator_->add_evaluator(&right_loop);
    ...
    auto left_heel_evaluator = multibody::WorldPointEvaluator(
        ...
        Vector3d::Zero(), {0, 1, 2});
    left_contact_evaluator_->add_evaluator(&left_toe_evaluator);
    left_contact_evaluator_->add_evaluator(&left_heel_evaluator);
    ...
    auto right_heel_evaluator = multibody::WorldPointEvaluator(
        ...
        Vector3d::Zero(), {0, 1, 2});
    right_contact_evaluator_->add_evaluator(&right_toe_evaluator);
    right_contact_evaluator_->add_evaluator(&right_heel_evaluator);
...
TEST_F(ContactEstimationTest, solveFourbarLinkageTest) {
  ...
  CassieStateEstimator estimator(
      plant, &fourbar_evaluator, &left_contact_evaluator,
      &right_contact_evaluator);
  ...
  q_init << VectorXd::Zero(7), -0.084017, 0.0826151, -0.00120735, 0.00217829,
      0.366012, 0.365803, -0.6305, -0.630502, 0.00205363, 0.00205356, 0.838878,
      0.838882, 0, 0, 0.205351, 0.20456;
  ...
  std::cout << calc_left_heel_spring << ", " << calc_right_heel_spring << std::endl;
  ...
  multibody::MultibodyProgram<double> program(plant);
  auto q = program.AddPositionVariables();
  auto u = program.AddInputVariables();
  auto lambda = program.AddConstraintForceVariables(fourbar_evaluator);
  program.AddKinematicConstraint(fourbar_evaluator, q);
