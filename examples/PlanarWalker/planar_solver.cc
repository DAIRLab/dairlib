...
VectorXd SolvePlanarStandingConstraints(const RigidBodyTree<double>& tree, 
  ...
  auto constraint_contact = std::make_shared<PlanarStandingConstraint>(tree);
...
//  auto constraint_fixed_point = make_shared<PlanarFixedPointConstraint>(
...
//  //DRAKE_DEMAND(CheckPlanarFixedPointConstraints(plant, q_sol, u_sol, lambda_sol));
//
//  vector<VectorXd> sol;
//  sol.push_back(q_sol);
//  sol.push_back(u_sol);
...
//
//  auto constraint = std::make_shared<PlanarFixedPointConstraint>(plant, 2);
...
vector<VectorXd> SolvePlanarFixedPointAndStandingConstraints(const RigidBodyPlant<double>& plant, 
  ...
  auto constraint_standing = make_shared<PlanarStandingConstraint>(
      plant.get_rigid_body_tree());


  auto constraint_fixed_point = make_shared<PlanarStandingFixedPointConstraint>(
  ...
  for (int i = 0; i < num_contacts; i++) {
    ...
    prog.AddConstraint(lambda(i*2 + 1) <= mu*lambda(i*2));
    prog.AddConstraint(-lambda(i*2 + 1) <= mu*lambda(i*2));
  }
  ...
  prog.AddQuadraticCost(lambda(0)*lambda(0) + lambda(2)*lambda(2));
...
PlanarStandingConstraint::PlanarStandingConstraint(const RigidBodyTree<double>& tree,
...
void PlanarStandingConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& q,
...
void PlanarStandingConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& q,
  ...
  //std::cout << phi_total << std::endl;
  //std::cout << "---------------------------------------------" << std::endl;
  //std::cout << normal_total << std::endl;
  //std::cout << "---------------------------------------------" << std::endl;
  //std::cout << xA_total << std::endl;
  //std::cout << "---------------------------------------------" << std::endl;
  //std::cout << xB_total << std::endl;
  //std::cout << "---------------------------------------------" << std::endl;

  //for(auto t:idxA_total) {
  //  std::cout << t << " ";
  //}
  //std::cout << std::endl;
  ...
  // 2 contacts for the planar model 
  const int num_contacts = 2;
  ...
  auto y_tmp = initializeAutoDiff(VectorXd::Zero(2));
...
void PlanarStandingConstraint::DoEval(const Eigen::Ref<const VectorX<Variable>>& x, 
...
//PlanarFixedPointConstraint::PlanarFixedPointConstraint(const RigidBodyPlant<double>& plant,
//                                                       int num_constraint_forces,
//                                                       bool print_debug,
//                                                       const std::string& description):
...
//
//
//void PlanarFixedPointConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& q_u_l,
//                                        Eigen::VectorXd* y) const {
//  AutoDiffVecXd y_t;
...
//}
//
//void PlanarFixedPointConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& q_u_l,
//                                        AutoDiffVecXd* y) const {
//
//
...
//}
//
//void PlanarFixedPointConstraint::DoEval(const Eigen::Ref<const VectorX<Variable>>& q_u_l, 
//                            VectorX<Expression>*y) const {
//
//  throw std::logic_error(
...
PlanarStandingFixedPointConstraint::PlanarStandingFixedPointConstraint(const RigidBodyPlant<double>& plant,
                                                                       const RigidBodyPlant<AutoDiffXd>& plant_autodiff,
                                                                       int num_constraint_forces,
                                                                       bool print_debug,
                                                                       const std::string& description):
...
void PlanarStandingFixedPointConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& q_u_l,
                                        Eigen::VectorXd* y) const {
...
void PlanarStandingFixedPointConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& q_u_l,
                                        AutoDiffVecXd* y) const {
  ...
  *y = planar_plant.CalcMVdotPlanarStanding(x, u, lambda);
...
void PlanarStandingFixedPointConstraint::DoEval(const Eigen::Ref<const VectorX<Variable>>& q_u_l, 
                            VectorX<Expression>*y) const {
