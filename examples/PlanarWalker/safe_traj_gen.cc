...
Vector3d SafeTrajGenerator::solveQP(Vector3d CoM_wrt_foot, Vector3d dCoM,
  ...
  acceleration_constraint_->UpdateCoefficients(f.segment<1>(1),
                                               g.block<1, 2>(1, 0));
...
void SafeTrajGenerator::CalcTraj(const Context<double> &context,
  ...
  Vector3d state_dot =
      solveQP(CoM_wrt_foot, dCoM, pt_on_swing_foot);

  Polynomiald t("t");
  drake::MatrixX<Polynomiald> polynomial_matrix;
  polynomial_matrix(0, 0) = CoM_wrt_foot_x +
                            dCoM(0) * t +
                            state_dot(1) * t * t;
...
PiecewisePolynomial<double> SafeTrajGenerator::createSplineForSwingFoot(
  ...
  double duration_of_stance = end_time_of_this_interval - start_time_of_this_interval;
