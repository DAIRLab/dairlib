...
int do_main(int argc, char* argv[]) {
  ...
  drake::parsers::urdf::AddModelInstanceFromUrdfFileToWorld(
      "PlanarWalker.urdf", drake::multibody::joints::kFixed, tree.get());
  ...
  AutoDiffVecXd pts =
      tree.get()->transformPoints(cache_autodiff, xB.col(0), idxB[0], 0);
  cout << "***********PTS ***********" << endl;
  cout << pts << endl;
  ...
  for (int i = 0; i < num_pts; ++i) {
    JA = tree.get()->transformPointsJacobian(cache, xA.col(i), idxA.at(i), 0,
                                             true);
    JB = tree.get()->transformPointsJacobian(cache, xB.col(i), idxB.at(i), 0,
                                             true);
  ...
  // xB.col(0), idxB.at(0), 0); VectorXd JdotV =
  ...
  auto Jdiff = tree.get()->transformPointsJacobian(
      cache_autodiff, xBc0.template cast<AutoDiffXd>(), idxB.at(0), 0, true);
  ...
  // idxB.at(0), 0);
  AutoDiffVecXd JdotV = tree.get()->transformPointsJacobianDotTimesV(
      cache_autodiff, xBc0, idxB.at(0), 0);
  ...
  // see
  // https://github.com/RobotLocomotion/drake/blob/master/multibody/rigid_body_constraint.cc#L1930~L1959
