...
void visualizeGait(std::string file, int steps, double rate) {
  ...
  auto leftFootConstraint =
      DirconPositionData<double>(tree, leftLegIdx, pt, isXZ);
  auto rightFootConstraint =
      DirconPositionData<double>(tree, rightLegIdx, pt, isXZ);
  ...
  for (int j = 0; j < steps; j++) {
    double t_start = pp_xtraj.end_time() +
                     (j - 1) * (pp_xtraj.end_time() - pp_xtraj.start_time());
  ...
  auto state_source =
      builder.AddSystem<drake::systems::TrajectorySource>(pp_xtraj_repeated);
  auto publisher =
