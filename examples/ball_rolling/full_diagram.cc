...
#include "examples/ball_rolling/parameters/franka_c3_controller_params.h"
...
DEFINE_string(controller_settings,
              "examples/ball_rolling/parameters/franka_c3_controller_params.yaml",
...
int DoMain(int argc, char* argv[]) {
  ...
  FrankaLcmChannels lcm_channel_params =
      drake::yaml::LoadYamlFile<FrankaLcmChannels>(
          "examples/ball_rolling/parameters/lcm_channels_simulation.yaml");
  // load parameters
  FrankaSimParams sim_params = drake::yaml::LoadYamlFile<FrankaSimParams>(
      "examples/ball_rolling/parameters/franka_sim_params.yaml");
  ...
  OSCGains gains = drake::yaml::LoadYamlFile<OSCGains>(
      FindResourceOrThrow(
          "examples/ball_rolling/parameters/franka_osc_controller_params.yaml"),
      {}, {}, yaml_options);
  ...
  SamplingC3SamplingParams sampling_params =
      drake::yaml::LoadYamlFile<SamplingC3SamplingParams>(
          "examples/ball_rolling/parameters/sampling_params.yaml");
  SamplingC3TrajectoryParams trajectory_params =
      drake::yaml::LoadYamlFile<SamplingC3TrajectoryParams>(
          "examples/ball_rolling/parameters/trajectory_params.yaml");
  ...
  drake::solvers::SolverOptions osc_solver_options =
      drake::yaml::LoadYamlFile<solvers::SolverOptionsFromYaml>(
          FindResourceOrThrow(
              "examples/ball_rolling/parameters/franka_osc_qp_settings.yaml"))
          .GetAsSolverOptions(drake::solvers::OsqpSolver::id());
...
//                           plant_franka.GetFrameByName("ground"), X_F_G_franka);
  ...
  contact_geoms["SPHERE"] = sphere_geoms;
  ...
  ee_contact_pairs.push_back(SortedPair(contact_geoms["EE"], contact_geoms["SPHERE"]));
  //   Creating a list of contact pairs for the jack and the ground
  std::vector<SortedPair<GeometryId>> ground_contact {SortedPair(contact_geoms["SPHERE"], contact_geoms["GROUND"])};
...
//   TODO: When coming back to fix this file, make sure to change names to object/jack wherever necessary
  ...
  control_target->SetRemoteControlParameters(
      ...
      trajectory_params.lookahead_step_size, trajectory_params.lookahead_angle, trajectory_params.max_step_size,
