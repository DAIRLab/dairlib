...
void RegisterRigidGround(MultibodyPlant<double>* plant) {
  ...
  rigid_proximity_props.AddProperty(drake::geometry::internal::kHydroGroup,
                                    drake::geometry::internal::kRezHint, 0.001);
  /* Set up a ground. */
  Box ground{4, 4, 4};
...
DeformableBodyId RegisterRoundBelt(
  ...
  /* Load the round belt mesh and apply scaling. */
  const std::string round_belt_vtk = dairlib::FindResourceOrThrow(
      "examples/belt_assembly/urdf/round_belt_coarse.vtk");
  auto round_belt_mesh = std::make_unique<Mesh>(round_belt_vtk, scale);
  auto round_belt_instance = std::make_unique<GeometryInstance>(
      X_WB, std::move(round_belt_mesh), model_name);
  ...
  round_belt_instance->set_proximity_properties(deformable_proximity_props);
  ...
  return model->RegisterDeformableBody(
      std::move(round_belt_instance), deformable_config, unused_resolution_hint);
}
