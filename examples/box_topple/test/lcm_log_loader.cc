...
int DoMain(int argc,  char* argv[]) {
  ...
  std::cout<<"LOG LOADER FOR BOX EXAMPLE" << std::endl;
...
std::vector<SortedPair<GeometryId>> ground_object_contact_pairs;
  ...
  DrawAndSaveDiagramGraph(
    *plant_diagram, "examples/box_topple/test/franka_c3_plant_in_log_loader");
  ...
  DrawAndSaveDiagramGraph(
    *owned_diagram, "examples/box_topple/test/franka_c3_controller_in_log_loader");
...
void PythonFriendlyVectorOfVectorXdToFile(
  char* name, std::vector<Eigen::VectorXd> some_vector) {
  std::string filename = "/home/sharanya/workspace/dairlib/examples/box_topple/test";
  filename += "/tmp/";
  filename += name;
  filename += ".txt";
