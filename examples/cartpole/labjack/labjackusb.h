...
//
//  1.10 - Changed the LABJACK_HANDLE to a void * (previously int)
...
int LJUSB_OpenAllDevices(LABJACK_HANDLE* devHandles, UINT* productIds, UINT maxDevices);
...
LABJACK_HANDLE LJUSB_OpenDevice(UINT DevNum, unsigned int dwReserved, unsigned long ProductID);
...
bool LJUSB_ResetConnection(LABJACK_HANDLE hDevice);
...
unsigned long LJUSB_Write(LABJACK_HANDLE hDevice, const BYTE *pBuff, unsigned long count);
...
unsigned long LJUSB_Read(LABJACK_HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
unsigned long LJUSB_Stream(LABJACK_HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
unsigned long LJUSB_WriteTO(LABJACK_HANDLE hDevice, const BYTE *pBuff, unsigned long count, unsigned int timeout);
...
unsigned long LJUSB_ReadTO(LABJACK_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout);
...
unsigned long LJUSB_StreamTO(LABJACK_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout);
...
void LJUSB_CloseDevice(LABJACK_HANDLE hDevice);
// Closes the handle of a LabJack USB device.

bool LJUSB_IsHandleValid(LABJACK_HANDLE hDevice);
// Returns true if the handle is valid; this is, it is still connected to a
// device on the system.

unsigned short LJUSB_GetDeviceDescriptorReleaseNumber(LABJACK_HANDLE hDevice);
// Returns the device's release number (binary-coded decimal) stored in the
// device descriptor.
// hDevice = The handle for your device.

unsigned long LJUSB_GetHIDReportDescriptor(LABJACK_HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
unsigned long LJUSB_BulkRead(LABJACK_HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count);
...
unsigned long LJUSB_BulkWrite(LABJACK_HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count);
...
bool LJUSB_AbortPipe(LABJACK_HANDLE hDevice, unsigned long Pipe);
