# plots time trajectories of positions, velocities and accelerations of all 3 sims on same graph
...
def main():
    ...
    new_x1 = x1[:, np.where(t1 == common_times[0])[0][0]].reshape(45, 1)
    new_x2 = x2[:, np.where(t2 == common_times[0])[0][0]].reshape(45, 1)
    new_x3 = x3[:, np.where(t3 == common_times[0])[0][0]].reshape(45, 1)
    new_u1 = u1[:, np.where(t1 == common_times[0])[0][0]].reshape(10, 1)
    new_u2 = u2[:, np.where(t2 == common_times[0])[0][0]].reshape(10, 1)
    new_u3 = u3[:, np.where(t3 == common_times[0])[0][0]].reshape(10, 1)
