...
class CfMpfcOscDiagram : public drake::systems::Diagram<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(CfMpfcOscDiagram);

  // TODO (@Brian-Acosta) Add factory method for getting the plant
  CfMpfcOscDiagram(const drake::multibody::MultibodyPlant<double>& plant,
  ...
  [[nodiscard]] const InputPort<double>& get_input_port_mpc_output() const {
    return get_input_port(input_port_cf_mpc_output_);
  }
 ...
 private:
  ...
  std::unique_ptr<TransTaskSpaceTrackingData> stance_foot_data_for_leg_len;
  ...
  std::unique_ptr<DistanceTrackingData> leg_length_data;
  ...
  drake::systems::InputPortIndex input_port_cf_mpc_output_;
