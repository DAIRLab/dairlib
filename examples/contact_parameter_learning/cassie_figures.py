...
# saved_data = {'sensitivity_analysis' : figure_data_directory + '2021_09_09_15.pkl'}
  ...
  # saved_data = {'sensitivity_analysis' : figure_data_directory + '2022_02_22_18_mu.pkl'}
...
def make_gridded_sensitivity_analysis_figure(optimizer, sim_type, optimal_params, damp_key):
  ...
  #
  # np.save('X_' + format_sim_name(sim_type) + '_grid', X)
  # np.save('Y_' + format_sim_name(sim_type) + '_grid', Y)
  # np.save('Z_' + format_sim_name(sim_type) + '_grid', Z)

  cmaps = {'Drake': 'Blues',
           'MuJoCo': 'Reds',
           'Bullet': 'YlOrBr'}

  X = np.load('X_' + format_sim_name(sim_type) + '_grid.npy')
  Y = np.load('Y_' + format_sim_name(sim_type) + '_grid.npy')
  Z = np.load('Z_' + format_sim_name(sim_type) + '_grid.npy')

  cmap = plt.cm.get_cmap(cmaps[format_sim_name(sim_type)])
  cmap = cmap.reversed()
  ...
  plt.ylabel('Normalized Damping $b / b^{*}$')
  plt.title(format_sim_name(sim_type) + ' Sensitivity')
  plt.savefig(sim_type + '_cassie_grid')
  ps.save_fig(sim_type + '_cassie_grid')
...
def get_sensitivity_analysis_results(ids, param_ranges):
  ...
  for param_id in ids:
    ...
    eval_sim = sa.get_cassie_sim(param_id, optimizer)
    # params_range = get_cassie_params_range(sim_type)
    # import pdb; pdb.set_trace()
    params = load_params(sim_type)
...
def make_stiffness_sensitivity_analysis_figure(use_saved_data=False, save_figs=False):
  ...
  # ticks = plt.xticks()
  # import pdb; pdb.set_trace()
  plt.ylabel('Average Error ($e_{cassie}$)')
  plt.ylim((20, 70))
  plt.xlim((0.01, 10))
  if save_figs :
...
def make_friction_sensitivity_analysis_figure(use_saved_data=False, save_figs=False):
  ...
  for param_id in ids:
    sim_type = param_id.split('_')[0]
    # param = sa.load_cassie_params(param_id)
    param = load_params(sim_type)
    params[param_id] = param
    params_ranges[param_id] = sa.get_friction_range(sim_type, param[mu_keys[param_id]])
