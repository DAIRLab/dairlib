...
test_state = np.array([0.18629883,  0.02622872,  0.283257, -0.52503014,  0.39360754,
       -0.29753734, -0.67794127,  0.01438053,  1.29095332, -0.21252927,
        1.46313532, -.85439428,  0.86961928]).reshape(-1, 1)
...
def test_isaac():
    isaac_sim = isaac_cube_sim.IsaacCubeSim(visualize=True)
    isaac_sim.init_sim(isaac_cube_sim.default_isaac_contact_params)
    while True:
        _ = isaac_sim.get_sim_traj_initial_state(
            test_state, 300, cube_sim.CUBE_DATA_DT)
