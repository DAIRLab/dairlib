...
TrajectoryLcmParser::TrajectoryLcmParser(CubeFlipVisualizerParams& vis_params)
: num_trajectories_(vis_params.ic3_num_iters),
  N_(vis_params.trajectory_length) {
    ...
    for (int i = 0; i < num_trajectories_; i += vis_params.iter_downsampling_factor) {
      ...
      auto* port = 
        &this->DeclareAbstractOutputPort(
          ...
          [this, i, vis_params](const drake::systems::Context<double>& context,
            ...
            this->GetTrajectory(context, &traj, i, step);
          });
  ...
  void TrajectoryLcmParser::GetTrajectory(
    ...
    if (trajectory.HasTrajectory(trajectory_i_name)) {
      ...
      MatrixXd orientations = data.middleRows(cube_orientation_index, 4);
      MatrixXd positions = data.middleRows(cube_position_index, 3);
      ...
      for (int i = 0; i < downsampled_cols; i++) {
          ...
          // if (orientations.col(i * step).isZero()) {
          //   std::cout << "BAD ORIENTATION AHHHH" << std::endl;
          // }
