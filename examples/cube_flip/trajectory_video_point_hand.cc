...
#include "examples/cube_flip/parameter_headers/trajectory_video_params.h"
#include "examples/cube_flip/lcm_pose_getter.h"
...
int do_main(int argc, char* argv[]) {
  ...
  TrajectoryVideoParams video_params =
      drake::yaml::LoadYamlFile<TrajectoryVideoParams>("examples/cube_flip/parameters/trajectory_video_params_point_hand.yaml");
  ...
  for (auto fingertip : ee_base_link_names) {
    plant.WeldFrames(plant.world_frame(),
                            plant.GetFrameByName(fingertip), RigidTransform<double>::Identity());
  }
  ...
  auto pose_getter = 
      builder.AddSystem<LcmPoseGetter>(video_params, "pose_getter", 1);
  ...
  drake::geometry::Rgba cube_color(video_params.trace_color[0], 
      video_params.trace_color[1], video_params.trace_color[2], 1.0);  // Solid light blue
