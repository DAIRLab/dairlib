...
WalkingFiniteStateMachine::WalkingFiniteStateMachine(
    const MultibodyPlant<double>& plant, vector<double> impact_times,
    double delay_time, bool contact_driven, int init_state)
    : plant_(plant),
...
EventStatus WalkingFiniteStateMachine::DiscreteVariableUpdate(
  ...
  if (delay_time_ < 0.0) {
    switch ((FSM_STATE)fsm_state(0)) {
      case (LEFT_FOOT):
        if (current_time >= impact_times_[LEFT_FOOT] + delay_time_) {
          fsm_state << RIGHT_FOOT;
          PrintState((FSM_STATE)fsm_state(0), contact_time(0), current_time);
          prev_time(0) = current_time;
        }
        break;
      case (RIGHT_FOOT):
        if (current_time >= impact_times_[RIGHT_FOOT] + delay_time_) {
    ...
    switch ((FSM_STATE)fsm_state(0)) {
      case (LEFT_FOOT):
        if (current_time > impact_times_[LEFT_FOOT]) {
          fsm_state << RIGHT_FOOT;
          PrintState((FSM_STATE)fsm_state(0), current_time, current_time);
          prev_time(0) = current_time;
        }
        break;
      case (RIGHT_FOOT):
        if (current_time > impact_times_[RIGHT_FOOT]) {
          fsm_state << LEFT_FOOT_2;
          PrintState((FSM_STATE)fsm_state(0), current_time, current_time);
          prev_time(0) = current_time;
        }
...
void WalkingFiniteStateMachine::PrintState(FSM_STATE fsm_state,
                                           double contact_time,
                                           double current_time) const {
  std::cout << "current time: " << current_time << std::endl;
  std::cout << "contact time: " << contact_time << std::endl;
  std::cout << "Setting fsm to: " << fsm_state << std::endl;
}
