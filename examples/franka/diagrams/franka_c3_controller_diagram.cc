...
FrankaC3ControllerDiagram::FrankaC3ControllerDiagram(
  ...
  ///
//  plant_for_lcs_ = std::make_unique<drake::multibody::MultibodyPlant<double>>(0.0);
//  AddMultibodyPlantSceneGraph(&plant_builder, std::make_unique<drake::multibody::MultibodyPlant<double>>(0.0), scene_graph_lcs_);
//  plant_for_lcs_, scene_graph_lcs_ = AddMultibodyPlantSceneGraph(&plant_builder, 0.0);
...
//  lcs_diagram_builder.plant()
  ...
  if (controller_params.scene_index > 0) {
    ...
    lcs_diagram_builder.plant().WeldFrames(
        ...
        lcs_diagram_builder.plant().GetFrameByName("support", left_support_index), T_S1_W);
    lcs_diagram_builder.plant().WeldFrames(
        lcs_diagram_builder.plant().world_frame(),
        lcs_diagram_builder.plant().GetFrameByName("support", right_support_index), T_S2_W);
  }
  lcs_diagram_builder.parser().AddModels(controller_params.tray_model);

  lcs_diagram_builder.plant().WeldFrames(lcs_diagram_builder.plant().world_frame(),
  ...
  plant_for_lcs_autodiff_ =
      drake::systems::System<double>::ToAutoDiffXd(robot_diagram_for_lcs_->plant());

//  auto plant_diagram = plant_builder.Build();
//  auto plant_diagram = plant_lcs_builder_->Build();
//  plant_for_lcs_diagram_context_ =
//      plant_diagram->CreateDefaultContext();
//  plant_for_lcs_context_ = std::move(plant_diagram->GetMutableSubsystemContext(
//      plant_for_lcs_, plant_for_lcs_diagram_context_.get()));
  robot_diagram_root_context_ = robot_diagram_for_lcs_->CreateDefaultContext();
  plant_for_lcs_autodiff_context_ = plant_for_lcs_autodiff_->CreateDefaultContext();
  ...
  if (controller_params.scene_index > 0) {
    ...
    std::vector<drake::geometry::GeometryId> right_support_contact_points =
        robot_diagram_for_lcs_->plant().GetCollisionGeometriesForBody(
            robot_diagram_for_lcs_->plant().GetBodyByName("support", right_support_index));
  ...
  auto reduced_order_model_receiver =
      builder.AddSystem<systems::FrankaKinematics>(
          *plant_franka_, plant_franka_context_.get(), *plant_tray_, plant_tray_context_.get(),
  ...
  auto plate_balancing_target =
      builder.AddSystem<systems::PlateBalancingTargetGenerator>(*plant_tray_, controller_params.end_effector_thickness, controller_params.near_target_threshold);
  ...
  auto lcs_factory = builder.AddSystem<systems::LCSFactorySystem>(
      robot_diagram_for_lcs_->plant(), robot_diagram_for_lcs_->mutable_plant_context(robot_diagram_root_context_.get()), *plant_for_lcs_autodiff_,
      plant_for_lcs_autodiff_context_.get(), contact_pairs, c3_options);
  ...
  auto c3_trajectory_generator =
      builder.AddSystem<systems::C3TrajectoryGenerator>(robot_diagram_for_lcs_->plant(),
                                                        c3_options);
...
//                  controller->get_input_port_radio());
  ...
  builder.ExportInput(franka_state_receiver->get_input_port(), "franka_state: lcmt_robot_output");
  builder.ExportInput(tray_state_receiver->get_input_port(), "tray_state: lcmt_object_state");
  builder.ExportInput(radio_to_vector->get_input_port(), "raw_radio");
  builder.ExportOutput(c3_trajectory_generator->get_output_port_actor_trajectory(), "actor_trajectory");
//  builder.ExportOutput(c3_trajectory_generator->get_output_port_object_trajectory(), "object_trajectory");
//  builder.ExportOutput(c3_output_sender->get_output_port_c3_force(), "c3_forces");
