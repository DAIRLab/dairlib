...
ControlRefineSender::ControlRefineSender(
  ...
  contact_jacobian_port_ =
      this->DeclareAbstractInputPort("contact_jacobian_full",
                                     drake::Value<MatrixXd>{})
          .get_index();
  ...
  dt_idx_ = this->DeclareAbstractState(drake::Value<double>(0));
  dt_history_idx_ =
      this->DeclareAbstractState(drake::Value<std::deque<double>>());
...
EventStatus ControlRefineSender::UpdateSolveTimeHistory(
  ...
  auto& dt_history =
      state->get_mutable_abstract_state<std::deque<double>>(dt_history_idx_);
  auto& prev_time = state->get_mutable_abstract_state<double>(prev_time_idx_);
  auto& dt = state->get_mutable_abstract_state<double>(dt_idx_);
  ...
  // get approximate compute time for state_estimation->forward kinematics->c3
  // computation
  if (dt_history.empty()) {
  ...
  auto system_scaling_pair = LCSFactory::LinearizePlantToLCS(
      ...
      c3_options_.num_friction_directions, c3_options_.mu, dt, c3_options_.N,
      contact_model);
  ...
  auto flag = LCPSolver.SolveLcpLemkeRegularized(
      lcs_system.F_[0],
      lcs_system.E_[0] * scaling * x + lcs_system.c_[0] * scaling +
          lcs_system.H_[0] * scaling * u_C3,
      &lambda);
  ...
  x_next.SetFromVector(lcs_system.A_[0] * x + lcs_system.B_[0] * u_C3 +
                       lcs_system.D_[0] * lambda / scaling + lcs_system.d_[0]);
...
void ControlRefineSender::CalcTrackTarget(
    const Context<double>& context, TimestampedVector<double>* target) const {
  ...
  track_target << ee_orientation_target, x_next.head(3), VectorXd::Zero(3),
      x_next.segment(10, 3);
...
void ControlRefineSender::CalcFeedForwardTorque(
    const Context<double>& context, BasicVector<double>* torque_force) const {
  // all other calculations are done in EventStatus Update
  auto contact_jacobian =
      EvalAbstractInput(context, contact_jacobian_port_)->get_value<MatrixXd>();
