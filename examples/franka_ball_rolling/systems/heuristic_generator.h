...
class HeuristicGenerator : public drake::systems::LeafSystem<double> {
 public:
  ...
  /// end-effector) to the C3 block potential TODO: y_d contain the whole
  /// desired state, can seperate robot and object, position and velocity ports
  /// if needed
  const drake::systems::OutputPort<double>& get_output_port_target() const {
    return this->get_output_port(output_target_port_);
  }

  /// the second output port send out heuristicly determined tilt orientation to
