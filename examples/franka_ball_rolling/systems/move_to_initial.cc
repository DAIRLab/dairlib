...
EventStatus MoveToInitial::UpdateFirstMessageTime(
...
void MoveToInitial::CalcTarget(const Context<double>& context,
...
std::vector<Vector3d> MoveToInitial::move_to_initial_position(
    const Vector3d& start, const Vector3d& finish, double curr_time,
    double stabilize_time, double move_time) const {
...
Quaterniond MoveToInitial::move_to_initial_orientation(
    const Quaterniond& start_orientation, double tilt_degrees, double curr_time,
    double stabilize_time, double move_time) const {
  ...
  if (curr_time < stabilize_time + move_time) {
    ...
    RotationMatrix<double> rot_y =
        RotationMatrix<double>::MakeYRotation(tilt_degrees * PI / 180);
    Quaterniond orientation_d = (R_start * rot_y).ToQuaternion();
    return orientation_d;
  }
}
