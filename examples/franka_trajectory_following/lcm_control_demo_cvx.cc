...
#include "systems/robot_lcm_systems.h"
#include "systems/controllers/c3_controller_franka_cvx.h"
...
// new lcm file newly created for using Anitescu's Model, since we hard code the dimension
...
int DoMain(int argc, char* argv[]){
  ...
  MatrixXd Ginit = param.G * MatrixXd::Identity(nq+nv+nu+4*nc*num_balls+12, nq+nv+nu+4*num_balls*nc+12);
  MatrixXd Uinit = param.U_default * MatrixXd::Identity(nq+nv+nu+4*num_balls*nc+12, nq+nv+nu+4*num_balls*nc+12);
  Uinit.block(0,0,nq+nv,nq+nv) << 
    param.U_pos_vel * MatrixXd::Identity(nq+nv,nq+nv);
  Uinit.block(nq+nv+4*num_balls*nc+12, nq+nv+4*num_balls*nc+12, nu, nu) <<
  ...
  auto controller = builder.AddSystem<systems::controllers::C3Controller_franka_Convex>(
  ...
  auto state_force_sender = builder.AddSystem<systems::RobotC3Sender>(7, 3, 4*num_balls, 1);
