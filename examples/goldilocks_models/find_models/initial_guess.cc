    ...
    VectorXd& weight_vector,MatrixXd& solution_matrix)
{
  ...
  if(is_success == 1) {
    //extract past gamma
    VectorXd past_gamma = readCSV(dir + prefix
                                      + string("_task.csv"));
    ...
    VectorXd w_to_interpolate = readCSV(dir + prefix
                                            + string("_w.csv"));
    //concatenate the weight and solution for further calculation
    solution_matrix.conservativeResize(w_to_interpolate.rows(),solution_matrix.cols()+1);
...
//    get theta of current iteration and task of current sample
  ...
  VectorXd current_gamma = Eigen::Map<const VectorXd>(
