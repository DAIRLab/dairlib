...
namespace dairlib::goldilocks_models {
    int test_initial_guess(int argc, char* argv[]){
      ...
      for (iteration = 0; iteration <= iter; iteration++)
      {
        ...
        writeCSV(dir + to_string(iteration) + string("_theta_s.csv"), theta_s);
        writeCSV(dir + to_string(iteration) + string("_theta_sDDot.csv"), theta_sDDot);
        ...
        for (sample = 0; sample <= total_sample_num; sample++)
        {
          ...
          MatrixXd stride_length = (max_sl+min_sl)/2*MatrixXd::Ones(1,1)+
              (max_sl-min_sl)/2*MatrixXd::Random(1, 1);
          writeCSV(dir + prefix + string("stride_length.csv"),
                   stride_length);
          MatrixXd ground_incline = (max_gi+min_gi)/2*MatrixXd::Ones(1,1)
