...
int RunToySystem(drake::lcm::DrakeLcm& lcm) {
  ...
  const std::string plate_file_lcs = "examples/iC3/urdf/plate.sdf";
	const std::string cube_file_lcs = "examples/iC3/urdf/pancake.sdf";
  ...
  const std::string plate_file = "examples/iC3/urdf/plate.sdf";
	const std::string cube_file = "examples/iC3/urdf/pancake.sdf";
  ...
  if (toy_params.input_type == "open") {
    ...
    LCSFactory lcs_factory = LCSFactory(plant_for_lcs, plant_for_lcs_context, *plant_autodiff,
                                *plant_context_autodiff, c3_controller_options.lcs_factory_options);

    auto controller =
        builder.AddSystem<systems::iC3TrackingController>(
          plant_for_lcs, lcs_factory, c3_controller_options, ic3_options, xd, 
          0, A_x, lb_x, ub_x, A_u, lb_u, ub_u);
