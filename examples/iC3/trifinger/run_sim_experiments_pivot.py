#!/usr/bin/env python3
"""
Run batch simulation experiments for TriFinger Pivot task.

Features:
- No settling phase: launches sim, osc, and pivot hybrid MPC controller directly.
- Noise on initial object position (x, y) and yaw angle.
- Configures OSC neutral_position and LambdaEndEffectorW (1 on diagonal).
- Starts background LCM logging to /mnt/iC3_logs/sim_data/trifinger_pivot.
- Saves results to examples/iC3/trifinger/sim_experiment_results_pivot/sim_experiments_results_pivot_<idx>.csv.
...
LOG_FOLDER = "/mnt/iC3_logs/sim_data/trifinger_pivot"
OUTPUT_DIR = os.path.join(REPO, "examples/iC3/trifinger/sim_experiment_results_pivot")

# Task specific target orientation (Pivot: [w=0, x=1, y=0, z=0])
TARGET_QUAT = np.array([0.0, 1.0, 0.0, 0.0])
TARGET_XYZ = np.array([0.0, 0.0, 0.052])
EXAMPLE_IDX = 2

# Task specific OSC parameters for Pivot
OSC_NEUTRAL_POS = [
    [0.0, 0.07, 0.05],
    [0.07, -0.03, 0.05],
    [-0.07, -0.03, 0.05]
]
OSC_LAMBDA_W_DIAG = 1.0
...
def _run_single_episode(trial_idx, env):
    ...
    try:
        ...
        launch("trifinger_pivot_hybrid_mpc_controller", "trifinger_hybrid_mpc_controller",
...
# ======================================================================
def main():
    parser = argparse.ArgumentParser(description="Run TriFinger Pivot simulation experiments with noisy initial object pose.")
    ...
    output_csv = os.path.join(OUTPUT_DIR, f"sim_experiments_results_pivot{idx_str}.csv")
    ...
    print("==================================================")
    print("    TRIFINGER PIVOT SIMULATION EXPERIMENTS RUNNER ")
    print("==================================================")
