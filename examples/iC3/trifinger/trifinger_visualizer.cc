...
#include "examples/iC3/trifinger/parameter_headers/trifinger_sim_params.h"
...
int do_main(int argc, char* argv[]) {
  ...
  TrifingerSimParams sim_params = drake::yaml::LoadYamlFile<TrifingerSimParams>(
      "examples/iC3/trifinger/parameters/trifinger_sim_params.yaml");
  ...
  auto trifinger_state_sub =
      builder.AddSystem(LcmSubscriberSystem::Make<dairlib::lcmt_robot_output>(
          lcm_channel_params.trifinger_state_channel, lcm));
  ...
  if (sim_params.visualize_workspace) {
    double width = sim_params.world_x_limits[1] -
                   sim_params.world_x_limits[0];
    double depth = sim_params.world_y_limits[1] -
                   sim_params.world_y_limits[0];
    double height = sim_params.world_z_limits[1] -
                    sim_params.world_z_limits[0];
    Vector3d workspace_center = {
        0.5 * (sim_params.world_x_limits[1] +
               sim_params.world_x_limits[0]),
        0.5 * (sim_params.world_y_limits[1] +
               sim_params.world_y_limits[0]),
        0.5 * (sim_params.world_z_limits[1] +
               sim_params.world_z_limits[0])};
