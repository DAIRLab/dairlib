#include "drake/systems/framework/leaf_system.h"
#include <vector>
#include "Eigen/Dense"
#include "Eigen/Core"

namespace dairlib {
namespace systems {

class AdditionalCostsSender: public drake::systems::LeafSystem<double> {
	// This is a system that reads the all_sample_costs output of 
	// std::vector<Eigen::vector3d> from the sampling controller and outputs it as an lcm message.
 public:
  AdditionalCostsSender(std::string name = "additional_costs_sender");

  // Input ports
  const drake::systems::InputPort<double>& get_input_port_sample_costs() const {
    return this->get_input_port(additional_costs_input_port_);
  }

  // Output port
  const drake::systems::OutputPort<double>& get_output_port_sample_costs() const {
    return this->get_output_port(additional_costs_output_port_);
  }

 private:
  void OutputAdditionalCosts(
        const drake::systems::Context<double>& context,
        dairlib::lcmt_timestamped_saved_traj* output_costs) const;

  drake::systems::InputPortIndex additional_costs_input_port_;
  drake::systems::OutputPortIndex additional_costs_output_port_;
};

}  // namespace systems
}  // namespace dairlib
