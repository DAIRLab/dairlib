...
def synchronize_messages(
    ...
    save_current_figure('time_synch', store_folder=log_folder)
...
def inspect_mode_switching(messages_by_channel: dict, log_folder: str = None):
    ...
    save_current_figure('sample_sources', store_folder=log_folder)
...
def visualize_goal_success(messages_by_channel: dict, trajectory_params: dict,
    ...
    plt.tight_layout()
    save_current_figure('goal_success', store_folder=log_folder)
    ...
    plt.tight_layout()
    save_current_figure('time_hist', store_folder=log_folder)
    ...
    save_current_figure('time_vs_error', store_folder=log_folder)
...
class ResultsAnalyzer:
    ...
    # TODO
    def update_channels(self, new_channel_set: List[str]):
        """TODO"""
        pass # TODO synchronize again?
    ...
    def visualize_goal_success(self):
        ...
        for thresh_i, color in enumerate(colors):
            ...
            pdf = count / sum(count)
            cdf = np.cumsum(pdf)
            axs.plot(bins_count[1:], cdf, color=color,
                     label=f'{pos_thresholds[thresh_i]:.2f}' + \
                        f'm, {rad_thresholds[thresh_i]:.1f}rad')
        axs.legend(title='Success Thresholds')
        axs.set_xlabel('Time to Goal [s]')
