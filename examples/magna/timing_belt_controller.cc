#include "timing_belt_controller.h"
...
TimingBeltController::TimingBeltController(
    ...
    const std::vector<
        ...
        contact_geoms,
    const TimingBeltControllerParams& timing_belt_controller_params,
    bool verbose)
    : plant_(plant),
      context_(context),
      plant_ad_(plant_ad),
      ...
      timing_belt_controller_params_(timing_belt_controller_params),
      assembly_c3_options_(timing_belt_controller_params.assembly_c3_options),
      N_(timing_belt_controller_params.assembly_c3_options.N),
      dt_(timing_belt_controller_params.assembly_c3_options.dt),
      mpc_target_lcs_states_(timing_belt_controller_params.target_lcs_states),
      osc_target_poses_pre_mpc_(
          timing_belt_controller_params.osc_target_poses_pre_mpc),
      osc_target_poses_post_mpc_(
          timing_belt_controller_params.osc_target_poses_post_mpc),
      verbose_(verbose) {
  this->set_name("timing_belt_controller");
  ...
  drake::log()->info("n_x_: {}", n_x_);
  drake::log()->info("n_q_: {}", n_q_);
  drake::log()->info("n_v_: {}", n_v_);
  drake::log()->info("n_u_: {}", n_u_);
  drake::log()->info("Q rows: {}, cols: {}", Q_[0].rows(), Q_[0].cols());
  drake::log()->info("R rows: {}, cols: {}", R_[0].rows(), R_[0].cols());
  drake::log()->info("G rows: {}, cols: {}", G_[0].rows(), G_[0].cols());
  drake::log()->info("U rows: {}, cols: {}", U_[0].rows(), U_[0].cols());
  ...
  c3_intermediates_port_ = this->DeclareAbstractOutputPort(
                                   ...
                                   &TimingBeltController::OutputC3Intermediates)
                               .get_index();
  ...
  c3_solution_port_ =
      this->DeclareAbstractOutputPort("c3_solution", c3_solution,
                                      &TimingBeltController::OutputC3Solution)
          .get_index();
  ...
  osc_target_tracking_debug_port_ =
      this->DeclareAbstractOutputPort(
              ...
              &TimingBeltController::OutputOscTargetTrackingDebug)
          .get_index();
  ...
  this->DeclareForcedDiscreteUpdateEvent(&TimingBeltController::ComputePlan);
...
TimingBeltController::DiscreteStateData
TimingBeltController::ExtractDiscreteState(
...
void TimingBeltController::UpdateDiscreteState(
...
bool TimingBeltController::ShouldAdvanceToNextOSCTarget(
  ...
  // Check if target is reached using the flag
  if (!target_reached) {
    return false;
  }
...
drake::systems::EventStatus TimingBeltController::ComputePlan(
  ...
  if (!osc_target_poses_pre_mpc_.empty() &&
      ...
      state_data.current_target_idx <
          static_cast<int>(osc_target_poses_pre_mpc_.size())) {
    // Check if we just reached the target (first time)
    if (!pre_mpc_target_reached_ &&
        IsOSCTargetReached(x_lcs_curr, state_data.current_target_idx,
                           osc_target_poses_pre_mpc_)) {
      // Record the time and pose when target was first reached
      state_data.target_reached_time = t_context;
      pre_mpc_target_reached_ = true;
      pre_mpc_reached_position_ = x_lcs_curr.head(3);
      // Convert RPY to quaternion
      Eigen::Vector3d rpy = x_lcs_curr.segment(3, 3);
      drake::math::RollPitchYaw<double> rpy_obj(rpy);
      Eigen::Quaterniond q = rpy_obj.ToQuaternion();
      pre_mpc_reached_orientation_ << q.w(), q.x(), q.y(), q.z();
      std::cout << "Pre-MPC target " << state_data.current_target_idx
                << " reached at t=" << state_data.target_reached_time
                << std::endl;
    }

    // Check if we should advance to the next target (when dwelling)
    if (ShouldAdvanceToNextOSCTarget(t_context, state_data.current_target_idx,
  ...
  if (!osc_target_poses_post_mpc_.empty() &&
      ...
      state_data.post_mpc_target_idx <
          static_cast<int>(osc_target_poses_post_mpc_.size())) {
    // Check if we just reached the target (first time)
    if (!post_mpc_target_reached_ &&
        IsOSCTargetReached(x_lcs_curr, state_data.post_mpc_target_idx,
                           osc_target_poses_post_mpc_)) {
      // Record the time and pose when target was first reached
      state_data.post_mpc_target_reached_time = t_context;
      post_mpc_target_reached_ = true;
      post_mpc_reached_position_ = x_lcs_curr.head(3);
      // Convert RPY to quaternion
      Eigen::Vector3d rpy = x_lcs_curr.segment(3, 3);
      drake::math::RollPitchYaw<double> rpy_obj(rpy);
      Eigen::Quaterniond q = rpy_obj.ToQuaternion();
      post_mpc_reached_orientation_ << q.w(), q.x(), q.y(), q.z();
    ...
    // Check if we should advance to the next target (when dwelling)
    if (ShouldAdvanceToNextOSCTarget(t_context, state_data.post_mpc_target_idx,
    ...
    case AssemblyPhase::kMoveToTargetPreMPC:
      if (!osc_target_poses_pre_mpc_.empty() &&
        ...
        GenerateMoveToTargetTrajectory(
            ...
            &pre_mpc_target_reached_, &pre_mpc_reached_position_,
            &pre_mpc_reached_orientation_);
      }
      break;
    ...
    case AssemblyPhase::kMoveToTargetPostMPC:
      if (!osc_target_poses_post_mpc_.empty() &&
        ...
        GenerateMoveToTargetTrajectory(
            ...
            &post_mpc_target_reached_, &post_mpc_reached_position_,
            &post_mpc_reached_orientation_);
      }
      break;
...
bool TimingBeltController::IsOSCTargetReached(
  ...
  bool position_reached =
      ...
      (target_pose.radius > 0.0
           ? timing_belt_controller_params_.predefined_motion_position_tolerance
           : timing_belt_controller_params_
  ...
  bool orientation_reached =
      angle <
      timing_belt_controller_params_.predefined_motion_orientation_tolerance;
...
bool TimingBeltController::IsMpcTargetReached(
  ...
  bool position_reached = (x_lcs_curr.head(3) - x_lcs_des.head(3)).norm() <
                          timing_belt_controller_params_.mpc_position_tolerance;
  ...
  double angle = 2.0 * std::acos(dot_product);
  bool orientation_reached =
      angle < timing_belt_controller_params_.mpc_orientation_tolerance;

  return position_reached && orientation_reached;
}

void TimingBeltController::AddEETrajectoriesToLcm(
...
void TimingBeltController::GenerateMoveToTargetTrajectory(
  ...
  // If target_reached is true, hold the pose when target was first reached
  if (*target_reached) {
    ...
    knots.col(0) = *reached_position;
    knots.col(1) = *reached_position;
    timestamps[0] = t_context;
    timestamps[1] = t_context + dt_;

    Eigen::MatrixXd orientation_single = Eigen::MatrixXd::Zero(4, 2);
    orientation_single.col(0) = *reached_orientation;
    orientation_single.col(1) = *reached_orientation;
...
void TimingBeltController::GenerateMPCTrajectory(
  ...
  if (!is_warmup_complete_) {
    ...
    if (mpc_warmup_count_ >=
        timing_belt_controller_params_.mpc_warmup_iterations) {
...
void TimingBeltController::ConvertForcesToWorldFrame(
...
void TimingBeltController::OutputTrajExecute(
...
void TimingBeltController::OutputTrajPlannedKeypoints(
    const drake::systems::Context<double>& context,
    dairlib::lcmt_timestamped_saved_traj* output) const {
...
void TimingBeltController::OutputTrajPlanned(
    const drake::systems::Context<double>& context,
    dairlib::lcmt_timestamped_saved_traj* output) const {
...
void TimingBeltController::OutputGripperPosCommand(
    const drake::systems::Context<double>& context,
...
void TimingBeltController::OutputC3Forces(
    const drake::systems::Context<double>& context,
...
void TimingBeltController::OutputCurrentTargetLcsState(
    const drake::systems::Context<double>& context,
...
void TimingBeltController::OutputC3Intermediates(
    const drake::systems::Context<double>& context,
...
void TimingBeltController::OutputC3Solution(
    const drake::systems::Context<double>& context,
...
void TimingBeltController::OutputOscTargetTrackingDebug(
    const drake::systems::Context<double>& context,
