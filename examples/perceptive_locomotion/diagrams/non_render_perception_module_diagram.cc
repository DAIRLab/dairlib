#include "non_render_perception_module_diagram.h"
...
NonRenderPerceptionModuleDiagram::NonRenderPerceptionModuleDiagram(
  ...
  DRAKE_DEMAND(depth_sensor_info.count("pelvis_depth") == 1);
  ...
  state_estimator_ = builder.AddSystem<systems::CassieStateEstimator>(
      *plant_, joint_offsets_, -1 //2 flag for the contact modes
  );
  ...
  for (const auto& sensor_pose : mapping_params.sensor_poses) {
    ...
    auto points_generator = builder.AddSystem<perception::SimulatedHmapPointServer<pcl::PointXYZRGBConfidenceRatio>>(
        *plant_,
        plant_->CreateDefaultContext(),
        mapping_params,
        depth_sensor_info.at(sensor_name), polygons);

    input_port_gt_state_ = builder.ExportInput(
        points_generator->get_input_port()
    );

    builder.Connect(
        points_generator->get_output_port(),
...
std::unique_ptr<NonRenderPerceptionModuleDiagram> NonRenderPerceptionModuleDiagram::Make(
    std::string elevation_mapping_params_yaml_path,
    std::map<std::string, drake::systems::sensors::CameraInfo> depth_sensor_info,
  ...
  plant->Finalize();
  return std::make_unique<NonRenderPerceptionModuleDiagram>(
      std::move(plant), elevation_mapping_params_yaml_path,
      depth_sensor_info, polygon_terrain, joint_offsets_yaml
  );
}

void NonRenderPerceptionModuleDiagram::InitializeEkf(
...
void NonRenderPerceptionModuleDiagram::InitializeElevationMap(
