...
RLOscDiagram::RLOscDiagram(
  ...
  auto rl_receiver = builder.AddSystem<MpfcOutputFromRL>();
  auto mpc_receiver = builder.AddSystem<AlipMpcOutputReceiver>();


  builder.Connect(*rl_receiver, *mpc_receiver);
  ...
  input_port_rl_ = builder.ExportInput(
      rl_receiver->get_input_port(),
      "footstep, fsm, prev_switch_time, next_switch_time"
  );
  ...
  this->set_name("rl_osc_diagram");
  DrawAndSaveDiagramGraph(*this);
}

void RLOscDiagram::SetSwingFootPositionAtLiftoff(
