...
def process_obj(base_name: str, urdf_dir: str, index: int, use_quats: bool):
    print(f"\nProcessing object: {base_name}")
    if not use_quats:
        print(f'\tRestricting to planar object orientations')
    output_dir = op.join(urdf_dir, base_name)
    os.makedirs(output_dir, exist_ok=True)

    obj_file = op.join(urdf_dir, f"{base_name}.obj")
    if not op.isfile(obj_file):
        obj_file = op.join(output_dir, f"{base_name}.obj")
        
    is_coarse = coarsify_obj(obj_file)

    # Create SDF file paths
    controller_sdf_path = op.join(output_dir, f"{base_name}_controller.sdf")
    ...
    # Generate SDFs
    obj_to_drake_sdf(obj_file, output_dir, j=index, use_quats=use_quats)
    make_sdf(obj_file, controller_sdf_path, use_quats)
...
def set_object_paths(index, base_name, output_dir, controller_yaml, vis_yaml, sim_yaml):
    ...
    controller_sdf_path = op.join(output_dir, f"{base_name}_controller.sdf")
    combined_sdf_path = op.join(output_dir, f"{base_name}.sdf")
...
def build_q_vector(num_objects: int, use_quats: bool) -> list:
    EE_POSITION = [0.01, 0.01, 0.01]
    OBJECT_CONFIG = [0.1, 0.1, 0.1, 0.1, 200, 200, 120] if use_quats else \
                    [200, 200, 120, 200]
    EE_LINEAR_VELOCITY = [15, 15, 10]
    OBJECT_VELOCITY = [0.05, 0.05, 0.05, 0.05, 0.05, 0.05] if use_quats else \
                      [0.05, 0.05, 0.05, 0.05]
    ...
    for _ in range(num_objects):
        q_vector.extend(OBJECT_VELOCITY)

    return q_vector

def update_c3_options(is_c3_plus, use_quats, samp_c3_options_yaml_path):
    ...
    samp_c3_options_yaml["q_vector"] = build_q_vector(num_objects, use_quats)
    samp_c3_options_yaml["q_vector_position"] = build_q_vector(num_objects, use_quats)

    n_config = 7 if use_quats else 4
    n_vel = 6 if use_quats else 4
    samp_c3_options_yaml["g_x"] = [950] * 3 + [1] * (n_config*num_objects) + [0.1] * (3 + n_vel*num_objects)
    ...
    if (is_c3_plus):
        ...
        samp_c3_options_yaml["u_x"] = [0] * (6 + ((n_config + n_vel) * num_objects))
    else:
        samp_c3_options_yaml["u_lambda_list"] = [[10] * (4 * calculate_contacts(num_objects, include_walls * num_objects))]
        if use_quats:
            samp_c3_options_yaml["u_x"] = [10] * 3 + [100, 100, 100, 100, 10, 10, 10] * num_objects + [8] * 3 + [1] * (n_vel*num_objects)
        else:
            samp_c3_options_yaml["u_x"] = [10] * 3 + [10, 10, 10, 10] * num_objects + [8] * 3 + [1] * (n_vel*num_objects)
    ...
    if (is_c3_plus):
        ...
        samp_c3_options_yaml["g_x_position"] = [950] * 3 + [1] * (n_config*num_objects) + [0.1] * (3 + n_vel*num_objects)
    else: 
    ...
    if (is_c3_plus):
        ...
        if use_quats:
            samp_c3_options_yaml["u_x_position"] = [10] * 3 + [100, 100, 100, 100, 10, 10, 10] * num_objects + [8] * 3 + [1] * (n_vel*num_objects)
        else:
            samp_c3_options_yaml["u_x_position"] = [10] * 3 + [10, 10, 10, 10] * num_objects + [8] * 3 + [1] * (n_vel*num_objects)
...
if __name__ == "__main__":
    ...
    for i in range(len(base_names)):
        process_obj(
            base_names[i], urdf_dir, i, config['orientation_is_quaternion'])
    ...
    num_objects = get_num_objects_from_yaml(controller_yaml_path)
    config["object_models"] = [""] * num_objects
    vis_yaml["object_vis_models"] = [""] * num_objects
    sim_yaml["object_models"] = [""] * num_objects

    for i in range(num_objects): 
        output_dir = op.join(urdf_dir, base_names[i])
        set_object_paths(i, base_names[i], output_dir, config, vis_yaml, sim_yaml)
    ...
    max_zs = [get_max_z_from_obj(op.join(urdf_dir, name, f"{name}.obj")) for name in base_names]
    min_zs = [get_min_z_from_obj(op.join(urdf_dir, name, f"{name}.obj")) for name in base_names]
    print(min_zs)
    ...
    if config['orientation_is_quaternion']:
        sim_yaml["q_init_objects"] = [[0.393, 0, 0, 0.92, 0.4 + (0.02 * index), -0.3 + (0.2 * index), 0.0] for index in range(num_objects)]
    else:
        sim_yaml["q_init_objects"] = [[0.4 + (0.02 * index), -0.3 + (0.2 * index), 0.0, 0.0] for index in range(num_objects)]
    ...
    use_quats = config["orientation_is_quaternion"]
    update_c3_options(is_c3_plus, use_quats, samp_c3_options_yaml_path)
    print(f"is_c3_plus: {is_c3_plus}")


    # Save pre-sized YAMLs
    save_yaml(controller_yaml_path, config)
