...
Eigen::MatrixXd Reposition(const int& n_q, const int& n_x, const int& N,
                           ...
                           const Eigen::VectorXd& repos_target,
  ...
  // Eigen::VectorXd current_ee_location = x_lcs.head(9);
  // Eigen::Vector3d current_object_location = x_lcs.segment(n_q - 3, 3);
  // Eigen::VectorXd curr_to_goal_vec = repos_target - current_ee_location;

  // // Get two unit vectors in the plane of the arc between the current and
  // goal
  // // ee locations.
  // Eigen::VectorXd v1 =
  //   (current_ee_location - current_object_location).normalized();
  // Eigen::VectorXd v2 = (repos_target - current_object_location).normalized();
  ...
  if (traj_type == RepositioningTrajectoryType::kStraightLine) {
...
void RepositionStraightLine(
    ...
    const Eigen::VectorXd& x_lcs, const Eigen::VectorXd& repos_target,
  ...
  next_lcs_state.head(9) = repos_target;
  next_lcs_state.segment(n_q, 9) = Eigen::VectorXd::Zero(9);
