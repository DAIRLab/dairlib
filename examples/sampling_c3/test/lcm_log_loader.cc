...
// bazel-bin/examples/sampling_c3/test/lcm_log_loader --demo_name=jacktoy
// /mnt/data2/sharanya/logs/2025/05_07_25/000000 1
// bazel-bin/examples/sampling_c3/test/lcm_log_loader --demo_name=push_t
// /mnt/data2/sharanya/logs/2025/05_07_25/000001 1
// bazel-bin/examples/sampling_c3/test/lcm_log_loader --demo_name=box_topple
// /mnt/data2/sharanya/logs/2025/05_07_25/000002 1
...
int DoMain(int argc, char* argv[]) {
  ...
  std::string log_number =
      log_folder.substr(log_folder.find_last_of("/") + 1, 6);
  ...
  // Sampling c3 options.
  // WARNING: ALL LOGS PRIOR TO MAY 15, 2025 WILL NOT HAVE A SAMPLING C3
  // OPTIONS. PLEASE USE AN OLDER VERSION OF THIS FILE TO LOAD LOGS COLLECTED
  // PRIOR TO MAY 15, 2025.
  std::string sampling_c3_options_path = log_filepath;
  std::string sampling_c3_options_path_replacement = "sampling_c3_options_";
  sampling_c3_options_path.replace(sampling_c3_options_path.find(to_replace),
                                   to_replace.length(),
  ...
  if (first_event != nullptr) {
    ...
    std::cout << "Initial event timestamp (s): " << u_init_time / 1e6
              << std::endl;
  } else {
  ...
  std::set<std::string> channels_to_check = {
      ...
      "DYNAMICALLY_FEASIBLE_CURR_ACTOR_PLAN",
      "C3_DEBUG_CURR",
      "IS_C3_MODE",
      "SAMPLE_LOCATIONS",
      "SAMPLE_COSTS",
      "DYNAMICALLY_FEASIBLE_BEST_PLAN",
      "DYNAMICALLY_FEASIBLE_BEST_ACTOR_PLAN"};
      ...
      if (event->timestamp >= time_into_log_in_microsecs + u_init_time) {
        ...
        if (message.decode(event->data, 0, event->datalen) > 0) {
          ...
          for (size_t i = 0;
            ...
            for (int j = 0; j < 3; j++) {
              sample_location(j) =
                  message.saved_traj.trajectories[0].datapoints[j][i];
            }
  ...
  std::cout << "x_lcs_actual from log : " << x_lcs_actual.transpose() << "\n"
            << std::endl;
  std::cout << "Available sample locations from log and corresponding costs: "
            << std::endl;
  std::cout << "Number of sample locations in log: "
            << sample_locations_in_log.size() << std::endl;
  for (int i = 0; i < sample_locations_in_log.size(); i++) {
    if (i < sample_costs_in_log.size()) {
      std::cout << "sample " << i << " : "
                << sample_locations_in_log[i].transpose() << " cost is "
                << sample_costs_in_log[i] << std::endl;
      if (i == 0) {
        std::cout << "Dynamically feasible plan is: " << std::endl;
        std::cout << "End effector position: "
                  << dyn_feas_curr_plan_ee_pos.transpose() << std::endl;
        std::cout << "Object orientation: "
                  << dyn_feas_curr_plan_obj_orientation.transpose()
                  << std::endl;
        std::cout << "Object position: "
                  << dyn_feas_curr_plan_obj_pos.transpose() << std::endl;
      }
      if (i == 1) {
        std::cout << "Dynamically feasible best plan is: " << std::endl;
        std::cout << "End effector position: "
                  << dyn_feas_best_plan_ee_pos.transpose() << std::endl;
        std::cout << "Object orientation: "
                  << dyn_feas_best_plan_obj_orientation.transpose()
                  << std::endl;
        std::cout << "Object position: "
                  << dyn_feas_best_plan_obj_pos.transpose() << std::endl;
      }
    } else {
      std::cout << "sample " << i << " : "
                << sample_locations_in_log[i].transpose() << " cost is "
  ...
  controller->get_input_port_target().FixValue(controller_context.get(),
                                               x_lcs_desired);
  controller->get_input_port_final_target().FixValue(controller_context.get(),
                                                     x_lcs_final_desired);
  ...
  // using cost type 3 in the push_T case. Eigen::VectorXd curr_sample_in_log =
  // x_lcs_actual; curr_sample_in_log.head(3) = sample_locations_in_log[0];
  ...
  if (FLAGS_demo_name == "push_t" || FLAGS_demo_name == "box_topple") {
    ...
    x_lcs_samples = GenerateEvenlySpacedSamplesAroundT(
        x_lcs_actual, sampling_params, N_VERTICAL, N_HORIZONTAL);
  } else if (FLAGS_demo_name == "jacktoy" ||
             FLAGS_demo_name == "ball_rolling") {
    // Directly call the GenerateEvenlySpacedSamplesAroundJack function
    auto [samples, angle_data] = GenerateEvenlySpacedSamplesAroundJack(
        x_lcs_actual, sampling_params, N_VERTICAL, N_HORIZONTAL);
  ...
  for (int i = 0; i < x_lcs_actuals.size(); i++) {
    ...
    controller->CalcForcedDiscreteVariableUpdate(*controller_context,
                                                 discrete_state.get());
    auto sample_costs_inside =
        controller->get_output_port_all_sample_costs()
...
std::vector<Eigen::VectorXd> GenerateEvenlySpacedSamplesAroundT(
  ...
  // the sampling region has an outer x and y limit. These are expressed in body
  // frame.
  ...
  // num_vertical means how many rows of samples we want to take. That is x
  // changes across each row. num_horizontal means how many columns of samples
  // we want to take. That is y changes across each column.
  ...
  // Generate uniform samples on a grid. Replace the end effector values in the
  // dummy with the sample values.
  ...
  double clearance = 0;  // 0.002;     // Maintain a clearance of 2mm from the
                         // T.
  ...
  std::vector<Eigen::VectorXd> filtered_samples_in_body_frame =
      unfiltered_samples_in_body_frame;
  ...
  for (const auto& sample : filtered_samples_in_body_frame) {
    ...
    sample_in_world_frame.head(3) =
        quat_object * sample.head(3) + object_position;
...
GenerateEvenlySpacedSamplesAroundJack(const Eigen::VectorXd& x_lcs,
  ...
  for (int i = 0; i < num_vertical; i++) {
    double elevation_theta =
        ...
        i * (max_angle_from_vertical - min_angle_from_vertical) /
            (num_vertical - 1);
    for (int j = 0; j < num_horizontal; j++) {
...
void PythonFriendlyVectorOfVectorXdToFile(
    char* name, std::vector<Eigen::VectorXd> some_vector) {
  std::string filename =
      "/home/sharanya/workspace/dairlib/examples/sampling_c3/push_t/test";
  filename += "/tmp/";
  filename += name;
  filename += ".txt";
