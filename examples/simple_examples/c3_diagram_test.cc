...
DEFINE_string(urdf, "examples/simple_examples/five_link_biped.urdf", "model urdf");
...
int DoMain(int argc, char *argv[]) {
 ...
 plant.Finalize();
  ...
  std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_ad = std::make_unique<MultibodyPlant<AutoDiffXd>>(plant);
  ...
  C3Controller controller = C3Controller(plant, context, plant_ad, context_ad, contact_geoms, num_friction_directions, mu, Q, R, U, xdesired);
