#include "fingertips_target_position_receiver.h"
...
FingertipTargetPositionReceiver::FingertipTargetPositionReceiver(
    ...
    : plant_(plant),
      ...
      target_pos_update_frequency_(target_pos_update_frequency) {
  this->set_name("fingertips_target_position_receiver");
  state_port_ =
  ...
  fingertips_target_position_port_ =
      this->DeclareAbstractInputPort(
              "fingertips_target_position",
              drake::Value<dairlib::lcmt_fingertips_target_position>{})
          .get_index();
  ...
  fingertips_target_port_ =
      this->DeclareVectorOutputPort(
              ...
              &FingertipTargetPositionReceiver::CopyToOutputFingertipsTarget)
          .get_index();
  ...
  DeclareForcedDiscreteUpdateEvent(
      &FingertipTargetPositionReceiver::DiscreteVariableUpdate);
...
FingertipTargetPositionReceiver::DiscreteVariableUpdate(
  ...
  const auto& fingertips_target_pos_lcm_msg =
      this->EvalInputValue<dairlib::lcmt_fingertips_target_position>(
          context, fingertips_target_position_port_);
  ...
  auto current_msg_timestamp = fingertips_target_pos_lcm_msg->utime;
  ...
  if (fingertips_target_pos_lcm_msg->is_delta_pos) {
    fingertips_target_pos +=
        Eigen::VectorXd::Map(fingertips_target_pos_lcm_msg->targetPos, 9);
  ...
  discrete_state->get_mutable_vector(fingertips_target_vel_idx_)
      .set_value(
          Eigen::VectorXd::Map(fingertips_target_pos_lcm_msg->targetVel, 9));
  discrete_state->get_mutable_vector(prev_target_timestamp_idx_)
      .set_value(
          (Eigen::VectorXd::Ones(1) * fingertips_target_pos_lcm_msg->utime));
  return drake::systems::EventStatus::Succeeded();
}

void FingertipTargetPositionReceiver::CopyToOutputFingertipsTargetTraj(
    const drake::systems::Context<double>& context,
  ...
  double target_pos_update_period = 1.0 / target_pos_update_frequency_;
  ...
  knots << start_time_traj, start_time_traj + target_pos_update_period;
...
void FingertipTargetPositionReceiver::CopyToOutputFingertipsTarget(
    const drake::systems::Context<double>& context,
...
void FingertipTargetPositionReceiver::CopyToOutputCurrentFingertipPositions(
    const drake::systems::Context<double>& context,
...
void FingertipTargetPositionReceiver::CopytoLCMCurrentFingertipPositions(
    const drake::systems::Context<double>& context,
...
void FingertipTargetPositionReceiver::CopytoLCMTargetFingertipPositions(
    const drake::systems::Context<double>& context,
