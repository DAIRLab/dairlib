...
static const int NUM_DATATYPES = 2;
...
class LcmTrajectoryTest : public ::testing::Test {
 protected:
  ...
  lcmt_trajectory_block traj_block_1_;
  lcmt_trajectory_block traj_block_2_;
...
TEST_F(LcmTrajectoryTest, TestSavingAndLoadingTrajectory) {
  ...
  lcmt_saved_traj loaded_lcm_traj = lcm_traj_.loadFromFile(TEST_FILEPATH);
  ...
  EXPECT_EQ(loaded_lcm_traj.metadata.datetime, metadata_.datetime);
  EXPECT_EQ(loaded_lcm_traj.metadata.name, TEST_NAME);
  EXPECT_EQ(loaded_lcm_traj.metadata.description, TEST_DESCRIPTION);
  EXPECT_EQ(loaded_lcm_traj.metadata.git_dirty_flag, metadata_.git_dirty_flag);
}

TEST_F(LcmTrajectoryTest, TestConstructorFromLcmTObject) {
  lcm_traj_.writeToFile(TEST_FILEPATH);

  LcmTrajectory loaded_traj = LcmTrajectory(lcm_traj_.loadFromFile(
                                TEST_FILEPATH));
  EXPECT_TRUE(
    loaded_traj.trajectories_[TEST_TRAJ_NAME_1].time_vector.isApprox(
