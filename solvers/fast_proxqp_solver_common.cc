...
FastProxQPSolver::FastProxQPSolver()
    : SolverBase(&id, &is_available, &is_enabled, &ProgramAttributesSatisfied,
                 &UnsatisfiedProgramAttributes) {}

FastProxQPSolver::~FastProxQPSolver() = default;

SolverId FastProxQPSolver::id() {
  static const never_destroyed<SolverId> singleton{"ProxQP"};
  return singleton.access();
}

bool FastProxQPSolver::is_enabled() { return true; }
...
bool CheckAttributes(
  ...
  if (!capabilities_match) {
    if (explanation) {
      *explanation = fmt::format(
          "FastProxQPSolver is unable to solve because {}.", *explanation);
    }
    return false;
  }
  if (required_capabilities.count(ProgramAttribute::kQuadraticCost) == 0) {
    if (explanation) {
      *explanation =
          "FastProxQPSolver is unable to solve because a QuadraticCost is required"
  ...
  if (nonconvex_quadratic_cost != nullptr) {
    if (explanation) {
      *explanation =
          "FastProxQPSolver is unable to solve because the quadratic cost " +
...
bool FastProxQPSolver::ProgramAttributesSatisfied(const MathematicalProgram& prog) {
  return CheckAttributes(prog, nullptr);
}

std::string FastProxQPSolver::UnsatisfiedProgramAttributes(
    const MathematicalProgram& prog) {
  std::string explanation;
