...
//#include "external/drake/common/_virtual_includes/autodiff/drake/common/eigen_autodiff_types.h"
...
C3Controller::C3Controller(const drake::multibody::MultibodyPlant<double> &plant,
                           ...
                           double mu, const vector<MatrixXd>& Q, const vector<MatrixXd>& R,
                           const vector<MatrixXd>& G, const vector<MatrixXd>& U,
                           const vector<VectorXd>& xdesired)
                           : plant_(plant),
                           context_(context),
                           plant_ad_(plant_ad),
  ...
  state_input_port_ = this->DeclareVectorInputPort("x, u, t", OutputVector<double>(num_positions, num_velocities, num_inputs)).get_index();

  control_output_port_ = this->DeclareVectorOutputPort("u, t", TimestampedVector<double>(num_inputs),
                                                       &C3Controller::CalcControl).get_index();
...
void C3Controller::CalcControl(const Context<double> &context,
  ...
  solvers::LCS system_ = solvers::LCSFactory::LinearizePlantToLCS(plant_,
                                                                                 context_,
                                                                                 plant_ad_,
                                                                                 context_ad_,
                                                                                 contact_geoms_,
  ...
  ///opt(system_, Q_, R_, G_, U_, xdesired_, options);

  /// calculate the input given x[i]
  VectorXd input = opt.Solve(x, delta, w);
