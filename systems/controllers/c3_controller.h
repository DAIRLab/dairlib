...
class C3Controller : public LeafSystem<double> {
 public:
  C3Controller(const drake::multibody::MultibodyPlant<double> &plant,
               ...
               double mu, const std::vector<Eigen::MatrixXd>& Q,
  ...
  const {
    return this->get_input_port(control_output_port_);
  }
 ...
 private:
  ...
  const drake::systems::Context<drake::AutoDiffXd>& context_ad_;
  const std::vector<drake::SortedPair<drake::geometry::GeometryId>>& contact_geoms_;
