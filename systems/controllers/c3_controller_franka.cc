...
C3Controller_franka::C3Controller_franka(
  ...
  state_output_port_ = this->DeclareVectorOutputPort(
          ...
          TimestampedVector<double>(47), &C3Controller_franka::CalcControl)
      .get_index();
...
VectorXd orientation_d = (rot * default_orientation).ToQuaternionAsVector4();
 ...
 ////////////////////////////////////////////////////////////////////////////////////
  ...
  // if (ts < roll_phase) {
    ...
    VectorXd st_desired(6 + optimal_sample_.size() + orientation_d.size() + ball_xyz_d.size() + ball_xyz.size() + true_ball_xyz.size() + 3 + 3 + 3);
    ...
    if(curr_ee_cost - min >= hyp){//75){ //heuristic threshold for if the difference between where I am and where I want to be is more than the threshold, then move towards that point
      //if(min < optimal_cost_) 
        ...
        //      std::cout<<"Moving up"<<std::endl;
          st_desired << next_point.head(3), orientation_d, optimal_sample_.tail(16), VectorXd::Zero(6), candidate_states[0].head(3), candidate_states[1].head(3), candidate_states[2].head(3), candidate_states[3].head(3), optimal_sample_.head(3), next_point.head(3);
...
// state_next = candidate_states[index];
  ...
  st_desired << state_next.head(3), orientation_d, state_next.tail(16), force_des.head(6), candidate_states[0].head(3), candidate_states[1].head(3), candidate_states[2].head(3), candidate_states[3].head(3), optimal_sample_.head(3), state_next.head(3);
