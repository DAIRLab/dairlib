...
C3Controller_franka::C3Controller_franka(
  ...
  state_output_port_ = this->DeclareVectorOutputPort(
          ...
          TimestampedVector<double>(56), &C3Controller_franka::CalcControl)
      .get_index();
 ...
 ////////////////////////////////////////////////////////////////////////////////////
    ...
    if (param_.num_samples == 0){
          ...
          st_desired << state_next.head(3), orientation_d, state_next.tail(16), force_des.head(6), candidate_states[0].head(3), 
              ...
              // For visualization to not give any issue at the moment, we need to set num_samples to 4
              // Isolate sampling from c3 solves on each.
              ...
              std::vector<solvers::LCS> candidate_lcs_objects;  // Items are appended to this vector.  Be careful about possible memory issues as a result.
              ...
              for (int i = 0; i < num_samples; i++) {
                ...
                candidate_lcs_objects.push_back(test_system);   // TO DO --> Plug in lcs ibject storing here. Parallelize for solver onwards.
              }
                ...
                if (param_.num_sample_threads == 0) {
                  ...
                  n_threads_to_use = param_.num_sample_threads;
                }
                ...
                solvers::C3MIQP opt_test(test_system, Qha, R_, G_, U_, traj_desired, options,
                                        warm_start_delta_, warm_start_binary_, warm_start_x_,
                                        warm_start_lambda_, warm_start_u_, true);
                // Should this stay here if we are parallelizing
                ///ADDING DELTA
                  if (options.delta_option == 1) {
                    /// reset delta and w (option 1)
                    delta = delta_reset;
                ...
                double cost = opt_test.CalcCost(test_state, optimalinputseq) + 1 * (test_state.head(2) - end_effector.head(2)).norm(); //+ std::pow((test_q[2]-ee[2]),2)); 
                cost_vector[i] = cost;
                // std::cout << "This is the cost of sample " << i << " : " << cost << std::endl;
                if (i == 0){
                  // Storing current position full solution for c3 if need be.
                  curr_fullsol_ = fullsol;
                  curr_ee_cost_ = cost;
                }
              }
              ...
              // Repositioning vs C3 decision
                    ...
                    if(curr_ee_cost_ - min >= hyp){//75){ //heuristic threshold for if the difference between where I am and where I want to be is more than the threshold, then move towards that point
                        //if(min < optimal_cost_) 
                          ...
                          std::cout<<"current cost : "<<curr_ee_cost_<<std::endl;
                    ...
                    auto system_scaling_pair2 = solvers::LCSFactoryFranka::LinearizePlantToLCS(
                        plant_f_, context_f_, plant_ad_f_, context_ad_f_, contact_pairs,
                        num_friction_directions_, mu_, 0.2);
                  ...
                  // state_next = candidate_states[index];
                    ...
                    if (curr_ee_cost_ - min >= param_.hyp3){
