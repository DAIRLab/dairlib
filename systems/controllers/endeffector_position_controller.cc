...
EndEffectorPositionController::EndEffectorPositionController(
    ...
    : plant_(plant),
      plant_world_frame_(plant_.world_frame()),
  ...
  joint_position_measured_port_ =
      this->DeclareVectorInputPort("joint_position_measured",
          ...
          .get_index();
  endpoint_position_commanded_port_ =
      this->DeclareVectorInputPort("endpoint_position_commanded",
                                   BasicVector<double>(3))
          .get_index();
  endpoint_orientation_commanded_port_ =
      this->DeclareVectorInputPort("endpoint_orientation_commanded",
...
void EndEffectorPositionController::CalcOutputTwist(
  ...
  VectorX<double> q_actual =
      this->EvalVectorInput(context, joint_position_measured_port_)
          ->CopyToVector();

  VectorX<double> x_desired =
      this->EvalVectorInput(context, endpoint_position_commanded_port_)
          ->CopyToVector();

  VectorX<double> orientation_desired =
      this->EvalVectorInput(context, endpoint_orientation_commanded_port_)
          ->CopyToVector();
  ...
  Eigen::Quaternion<double> quat_n_a =
      plant_
          .CalcRelativeTransform(*plant_context, plant_world_frame_,
                                 ee_joint_frame_)
          .rotation()
          .ToQuaternion();

  // Quaternion for rotation from world frame to desired end effector attitude.
  Eigen::Quaternion<double> quat_n_a_des =
      Eigen::Quaternion<double>(orientation_desired(0), orientation_desired(1),
                                orientation_desired(2), orientation_desired(3));
  ...
  VectorXd angularVelocityWF =
      plant_
          .CalcRelativeTransform(*plant_context, ee_joint_frame_,
                                 plant_world_frame_)
          .rotation() *
      angularVelocity;
