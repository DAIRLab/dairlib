...
class EndEffectorPositionController : public LeafSystem<double> {
 public:
  ...
  EndEffectorPositionController(const MultibodyPlant<double>& plant,
                                ...
                                Eigen::Vector3d ee_contact_frame, double k_p,
                                double k_omega, double max_linear_vel,
  ...
  const drake::systems::InputPort<double>& get_endpoint_orient_input_port()
      const {
    return this->get_input_port(endpoint_orientation_commanded_port_);
  }
  const drake::systems::OutputPort<double>& get_endpoint_cmd_output_port()
      const {
