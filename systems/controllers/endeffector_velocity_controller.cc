...
EndEffectorVelocityController::EndEffectorVelocityController(
  ...
  joint_position_measured_port_ =
      this->DeclareVectorInputPort("joint_position_measured",
                                   BasicVector<double>(num_joints_))
          .get_index();
  joint_velocity_measured_port_ =
      this->DeclareVectorInputPort("joint_velocity_measured",
                                   BasicVector<double>(num_joints_))
          .get_index();
  endpoint_twist_commanded_port_ =
      this->DeclareVectorInputPort("endpoint_twist_commanded",
...
void EndEffectorVelocityController::breakHere() const {
...
void EndEffectorVelocityController::CalcOutputTorques(
  ...
  plant_.CalcPointsPositions(*plant_context,
                             plant_.GetFrameByName("iiwa_link_4"), zeros3d,
  ...
  plant_.CalcJacobianSpatialVelocity(
      *plant_context, drake::multibody::JacobianWrtVariable::kQDot,
      plant_.GetFrameByName("iiwa_link_7"), ee_contact_frame_,
      plant_.GetFrameByName("iiwa_link_1"),
      plant_.GetFrameByName("iiwa_link_1"), &Jee);
  //  plant_.CalcFrameGeometricJacobianExpressedInWorld(      // TODO: replace
  //  with CalcJacobianSpatialVelocity?
  //      *plant_context, plant_.GetFrameByName("iiwa_link_7"),
  //      ee_contact_frame_, &Jee);
  ...
  Jf << 0,
      (sin(c) * sin(d)) /
          (pow(cos(b), 2) * pow(sin(d), 2) + pow(sin(b), 2) * pow(sin(c), 2) +
           pow(cos(c), 2) * pow(cos(d), 2) * pow(sin(b), 2) -
           2 * cos(b) * cos(c) * cos(d) * sin(b) * sin(d)),
      (pow(cos(b), 2) * cos(d) - cos(d) + cos(b) * cos(c) * sin(b) * sin(d)) /
          (pow(cos(b), 2) * pow(sin(d), 2) + pow(sin(b), 2) * pow(sin(c), 2) +
           pow(cos(c), 2) * pow(cos(d), 2) * pow(sin(b), 2) -
           2 * cos(b) * cos(c) * cos(d) * sin(b) * sin(d)),
      -(sin(b) * sin(c) * (cos(b) * cos(d) + cos(c) * sin(b) * sin(d))) /
          (pow((cos(b) * sin(d) - cos(c) * cos(d) * sin(b)), 2) +
           pow(sin(b), 2) * pow(sin(c), 2)),
      0,
      -(cos(d) * sin(b) - cos(b) * cos(c) * sin(d)) /
          (pow((1 - pow((cos(b) * cos(d) + cos(c) * sin(b) * sin(d)), 2)),
               (0.5))),
      -(sin(b) * sin(c) * sin(d)) /
          (pow((1 - pow((cos(b) * cos(d) + cos(c) * sin(b) * sin(d)), 2)),
               (0.5))),
      -(cos(b) * sin(d) - cos(c) * cos(d) * sin(b)) /
          (pow((1 - pow((cos(b) * cos(d) + cos(c) * sin(b) * sin(d)), 2)),
               (0.5))),
      1,
      (sin(c) * sin(d) * (cos(b) * cos(d) + cos(c) * sin(b) * sin(d))) /
          (pow(cos(d), 2) * pow(sin(b), 2) + pow(sin(c), 2) * pow(sin(d), 2) +
           pow(cos(b), 2) * pow(cos(c), 2) * pow(sin(d), 2) -
           2 * cos(b) * cos(c) * cos(d) * sin(b) * sin(d)),
      -(cos(b) * pow(cos(d), 2) - cos(b) + cos(c) * cos(d) * sin(b) * sin(d)) /
          (pow(cos(d), 2) * pow(sin(b), 2) + pow(sin(c), 2) * pow(sin(d), 2) +
           pow(cos(b), 2) * pow(cos(c), 2) * pow(sin(d), 2) -
           2 * cos(b) * cos(c) * cos(d) * sin(b) * sin(d)),
      -(sin(b) * sin(c)) /
  ...
  std::cout << "qdot" << std::endl << q_dot << std::endl;
  ...
  std::cout << "gains:" << k_d_ << std::endl;
  std::cout << "vel_d: " << vel_desired[0] << std::endl;
  std::cout << "vel_d: " << vel_desired[1] << std::endl;
  std::cout << "vel_d: " << vel_desired[2] << std::endl;

  std::cout << "vel_a: " << vel_actual(0, 0) << std::endl;
  std::cout << "vel_a: " << vel_actual(0, 0) << std::endl;
  std::cout << "vel_a: " << vel_actual(0, 0) << std::endl;
  ...
  if (std::isnan(commandedTorques[0])) {
    commandedTorques.setZero(num_joints_, 1);
  }
