...
class EndEffectorVelocityController : public LeafSystem<double> {
 public:
  ...
  EndEffectorVelocityController(const MultibodyPlant<double>& plant,
                                ...
                                Eigen::Vector3d ee_contact_frame, double k_d,
                                double k_r, double joint_torque_limit);
  ...
  const drake::systems::InputPort<double>& get_endpoint_twist_input_port()
      const {
    return this->get_input_port(endpoint_twist_commanded_port_);
  }
  const drake::systems::OutputPort<double>& get_endpoint_torque_output_port()
      const {
