...
std::vector<Eigen::VectorXd> generate_sample_states(
  ...
  if (sampling_params.sampling_strategy == RADIALLY_SYMMETRIC_SAMPLING) {
    ...
    // This method of sampling uses the plant to set various positions, and then
    // project samples in collision with the object to the closest point on the
    // object surface along with clearance and the end-effector's radius.
    for (int i = 0; i < num_samples; i++) {
      do {
...
Eigen::VectorXd generate_sample_on_grid(
  ...
  // check_collision function and have it directly modify this variable so it
  // can be accessed in this function without recomputing the collision check.
  ...
  while (true) {
    do {
      ...
      candidate_state.head(3) =
          ...
          object_position;
      // This is done because our param is expressed in world frame already so
      // the previous line gives the wrong z_value.
      candidate_state[2] = sampling_params.sampling_height;
    } while (!check_collision(n_q, n_v, n_u, candidate_state, plant, context,
                              plant_ad, context_ad, contact_geoms,
                              sampling_params, sampling_c3_options,
                              min_distance_index));

    // Once we find a sample in collision, project it to the surface of the
    // object.
    Eigen::VectorXd projected_state =
        project_to_surface(candidate_state, min_distance_index, sampling_params,
                           plant, context, contact_geoms);

    UpdateContext(n_q, n_v, n_u, plant, context, plant_ad, context_ad,
                  projected_state);
    if (check_collision(n_q, n_v, n_u, projected_state, plant, context,
                        plant_ad, context_ad, contact_geoms, sampling_params,
                        sampling_c3_options, min_distance_index)) {
      continue;  // If the projected state is still in collision, exit loop and
  ...
  while (true) {
    do {
      ...
      std::uniform_real_distribution<> dis_radius(
          sampling_params.sampling_radius - 0.03,
          sampling_params.sampling_radius);
...
bool check_collision(
  ...
  // This function returns a boolean value indicating whether the sample is in
  // collision with the object or not. If the sample is in collision with the
  // object, the function modifies a reference to the index of the closest pair
  // of contact geoms so that the projection function need not recompute the
  // closest pair.
