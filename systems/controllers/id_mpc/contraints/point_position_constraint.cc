...
PointPositionConstraint<T>::PointPositionConstraint(
    const ConstrainedDynamicsInfo &dynamics, const std::string &frame,
    const Vector3d &point_in_frame) : solvers::NonlinearConstraint<T>(
        ...
        frame_(dynamics.get_plant().GetBodyByName(frame).body_frame()),
...
void PointPositionConstraint<double>::EvaluateConstraint(
  ...
  if (active_) {
    ...
    plant_.CalcPointsPositions(*context_, frame_, point_, plant_.world_frame(), &phi);
    *y = phi - p;
  } else {
    *y = VectorXd::Zero(3);
  }
...
void PointPositionConstraint<AutoDiffXd>::EvaluateConstraint(
  ...
  if (active_) {
    ...
    plant_.CalcJacobianTranslationalVelocity(
        *context_, drake::multibody::JacobianWrtVariable::kQDot, frame_, point_,
