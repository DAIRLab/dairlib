...
ImpedanceController::ImpedanceController(
    ...
    drake::systems::Context<double>& context, const MatrixXd& K,
    const MatrixXd& B, const MatrixXd& K_null, const MatrixXd& B_null,
    const VectorXd& qd_null, bool gravity_compensation_flag)
    : plant_(plant),
      context_(context),
      K_(K),
      B_(B),
  ...
  /// TODO: contact_torque: 7D, lambda: 1D (temporary, only normal direction)
  ...
  /// setup event update for previous time recording, accumulating integral
  /// term and calculate torque
  this->DeclarePerStepUnrestrictedUpdateEvent(
      &ImpedanceController::UpdateControl);

  /// get impdeance_parameters
  ...
  MatrixXd rotational_integral_gain =
      impedance_param_.rotational_integral_gain.asDiagonal();
  MatrixXd translational_integral_gain =
      impedance_param_.rotational_integral_gain.asDiagonal();
  ...
  /// define franka joint torque limits and integral term limits
  torque_limits_ = impedance_param_.torque_limits;
  integral_limits_ = impedance_param_.integrator_clamp;

  /// timestamp recording settings
  prev_time_ = this->DeclareAbstractState(drake::Value<double>(0));

  /// final control torque
  tau_ = this->DeclareDiscreteState(n_);
}

/// Note the actual impedance control torque calculation is done in the even
/// status, it is because this way can avoid duplicating code for setting
/// integral torque term, we make previous time integrator internal drake
/// state, to replace the original mutable class variable, we also make final
/// impedance output torque tau a drake state, so that in CalControl we
/// directly set control torque by accessing the context and drake_state
void ImpedanceController::CalcControl(
  ...
  auto robot_output = (OutputVector<double>*)this->EvalVectorInput(
      context, franka_state_input_port_);
...
EventStatus ImpedanceController::UpdateControl(
  ...
  auto feedforward_command = (BasicVector<double>*)this->EvalVectorInput(
      context, contact_feedforward_input_port_);
  ...
  const drake::math::RigidTransform<double> H_W_EE = plant_.EvalBodyPoseInWorld(
      context_, plant_.GetBodyByName("end_effector_tip"));
  const RotationMatrix<double> R_W_EE = H_W_EE.rotation();
  Vector3d p = H_W_EE.translation();

  // build task space state vectors, task space state is the end-effecot
  ...
  // M_task_space is the task space intertia matrix, also is the capital Lambda
  // in the appendix of the paper
  MatrixXd M_inv = M.inverse();
  MatrixXd M_task_space =
      (J * M_inv * J.transpose()).inverse();  // apparent task space mass matrix

  // use integral term if it is helping, to turn down just set all integral gain
  // to be zero
  ...
  if (timestamp > 1 && enable_integral_) {
    ...
    if (SaturatedClamp(tau_int)) {
      std::cout << "clamped integrator torque" << std::endl;
      integrator = integrator_temp;
      ClampIntegratorTorque(tau_int);
    }
  }

  VectorXd tau =
      J.transpose() * M_task_space * (K_ * xtilde + B_ * xtilde_dot) + tau_int +
      C;

  // decide whether to apply gravity compensation or not, note the gravity sign
  // in drake
  if (gravity_compensation_flag_) {
    tau -= tau_g;
  }

  // compute nullspace projection, J_gen_inv means generalized inverse of
  // J_franka, used to define the null-space projector, here we use the
  // dynamical consistent generalized inverse, i.e. inverse weighted by the mass
  // matrix
  ...
  // contact feedforward compensation
  ...
  drake_state->get_mutable_discrete_state(integrator_).get_mutable_value()
      << integrator;
...
Vector3d ImpedanceController::CalcRotationalError(
    const RotationMatrix<double>& R, const Quaterniond& orientation_d) const {
  ...
  Vector3d error_quaternion_no_w(error_quaternion.x(), error_quaternion.y(),
                                 error_quaternion.z());
...
void ImpedanceController::ClampIntegratorTorque(VectorXd& tau) const {
  VectorXd thresholds = integral_limits_;
...
bool ImpedanceController::SaturatedClamp(const VectorXd& tau) const {
  VectorXd thresholds = integral_limits_;
  for (int i = 0; i < tau.size(); i++) {
    if (abs(tau(i)) > thresholds(i)) {
      return true;
