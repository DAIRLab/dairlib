...
#include "systems/controllers/mptc/mptc_tracking_data.h"

namespace dairlib::systems::controllers {

/// `ModularPassiveTrackingControl` takes in desired trajectory in world frame and
...
class ModularPassiveTrackingControl : public drake::systems::LeafSystem<double> {
 public:
  ...
  const drake::systems::OutputPort<double>& get_mptc_output_port() const {
    return this->get_output_port(mptc_output_port_);
  }
  const drake::systems::OutputPort<double>& get_mptc_debug_port() const {
    return this->get_output_port(mptc_debug_port_);
  }
  ...
  /// a new state)
  void AddTrackingData(mptc_data::MptcTrackingData* tracking_data, double t_lb = 0,
                       double t_ub = std::numeric_limits<double>::infinity());
  void AddConstTrackingData(
          mptc_data::MptcTrackingData* tracking_data, const Eigen::VectorXd& v, double t_lb = 0,
      double t_ub = std::numeric_limits<double>::infinity());
  std::vector<mptc_data::MptcTrackingData*>* GetAllTrackingData() {
    return tracking_data_vec_.get();
  }
    mptc_data::MptcTrackingData* GetTrackingDataByIndex(int index) {
    return tracking_data_vec_->at(index);
  }

  // MPTC LeafSystem builder
  void Build();

 private:
  // MPTC checkers and constructor-related methods
  void CheckCostSettings();
  void CheckConstraintSettings();

  // Get solution of MPTC QP
  ...
  void AssignMptcLcmOutput(const drake::systems::Context<double>& context,
  ...
  // Input/Output ports
  int mptc_debug_port_;
  int mptc_output_port_;
  int state_port_;
  int fsm_port_;
  ...
  std::unique_ptr<std::vector<mptc_data::MptcTrackingData*>> tracking_data_vec_ =
      std::make_unique<std::vector<mptc_data::MptcTrackingData*>>();
