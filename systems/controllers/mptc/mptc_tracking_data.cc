...
namespace dairlib::systems::controllers::mptc_data {

using multibody::makeNameToPositionsMap;
using multibody::makeNameToVelocitiesMap;
using multibody::CoriolisMatrixCalculator;

/**** MptcTrackingData ****/
MptcTrackingData::MptcTrackingData(const string& name, int n_y, int n_ydot,
                                 const MatrixXd& K_p, const MatrixXd& K_d,
                                 const MatrixXd& W,
                                 const MultibodyPlant<double>& plant_w_spr,
                                 const MultibodyPlant<double>& plant_wo_spr,
                                 const MultibodyPlant<AutoDiffXd>& plant_wo_spr_ad)
...
void MptcTrackingData::UpdateTrackingFlag(int finite_state_machine_state) {
  if (state_.empty()) {
...
void MptcTrackingData::SaveYddotCommandSol(const VectorXd& dv) {
  DRAKE_ASSERT(track_at_current_state_);
  yddot_command_sol_ = J_ * dv + JdotV_;
}

void MptcTrackingData::AddState(int state) {
...
void MptcTrackingData::CheckMptcTrackingData() {
  cout << "Checking " << name_ << endl;
  CheckDerivedMptcTrackingData();
...
ComTrackingData::ComTrackingData(const string& name, const MatrixXd& K_p,
                                 ...
                                 const MultibodyPlant<double>& plant_wo_spr,
                                 const MultibodyPlant<AutoDiffXd>& plant_wo_spr_ad)
    : MptcTrackingData(name, kSpaceDim, kSpaceDim, K_p, K_d, W, plant_w_spr,
                      plant_wo_spr, plant_wo_spr_ad) {}
...
TaskSpaceTrackingData::TaskSpaceTrackingData(
    ...
    const MultibodyPlant<double>& plant_wo_spr,
    const MultibodyPlant<AutoDiffXd>& plant_wo_spr_ad)
    : MptcTrackingData(name, n_y, n_ydot, K_p, K_d, W, plant_w_spr,
                      plant_wo_spr, plant_wo_spr_ad) {}
...
TransTaskSpaceTrackingData::TransTaskSpaceTrackingData(
    ...
    : TaskSpaceTrackingData(name, kSpaceDim, kSpaceDim, K_p, K_d, W,
                            plant_w_spr, plant_wo_spr, plant_wo_spr_ad) {}
...
/// TODO: @Brian-Acosta change to autodiff and update Jdot
...
JointSpaceTrackingData::JointSpaceTrackingData(
    ...
    const MultibodyPlant<double>& plant_wo_spr,
    const MultibodyPlant<AutoDiffXd>& plant_wo_spr_ad)
    : MptcTrackingData(name, 1, 1, K_p, K_d, W, plant_w_spr, plant_wo_spr, plant_wo_spr_ad) {}
...
void JointSpaceTrackingData::CheckDerivedMptcTrackingData() {
