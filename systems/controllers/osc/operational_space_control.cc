...
VectorXd OperationalSpaceControl::SolveQp(
  ...
  sol->solve_time_ = result.get_solver_details<OsqpSolver>().run_time;
...
void OperationalSpaceControl::AssignOscLcmOutput(
  ...
  potential_regularization_cost_names_and_vars {
      ...
      {"lambda_c_cost", osc_solution.lambda_c_sol_},
  ...
  qp_output.u_sol = CopyVectorXdToStdVector(osc_solution.u_sol_);
  qp_output.lambda_c_sol = CopyVectorXdToStdVector(osc_solution.lambda_c_sol_);
  qp_output.lambda_h_sol = CopyVectorXdToStdVector(osc_solution.lambda_h_sol_);
  qp_output.dv_sol = CopyVectorXdToStdVector(osc_solution.dv_sol_);
  qp_output.epsilon_sol = CopyVectorXdToStdVector(osc_solution.epsilon_sol_);
