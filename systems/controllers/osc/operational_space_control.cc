...
void OperationalSpaceControl::AddForceTrackingData(
  ...
  id_qp_.AddExternalForce(tracking_data->GetName(),
                          tracking_data->GetFrameEvaluator());
...
void OperationalSpaceControl::Build() {
  ...
  lambda_ee_sol_ = std::make_unique<Eigen::VectorXd>(id_qp_.ne());
  ...
  for (auto& force_tracking_data : *force_tracking_data_vec_) {
    // 3. external force tracking cost
    // Only supports a single force tracking data at the moment
    int ne = id_qp_.ne();
    DRAKE_DEMAND(force_tracking_data->GetWeight().rows() == ne);
    id_qp_.AddQuadraticCost(force_tracking_data->GetName(), force_tracking_data->GetWeight(),
...
//  id_qp_.UpdateDynamics(x_w_spr, active_contact_names, {force_tracking_data_vec_->at(0)->GetName()});
  ...
  for (auto& force_tracking_data : *force_tracking_data_vec_) {
    ...
    const auto& traj =
        input_traj->get_value<drake::trajectories::Trajectory<double>>();
    force_tracking_data->Update(traj, t);
  ...
  if (result.is_success()) {
    ...
    *lambda_ee_sol_ = result.GetSolution(id_qp_.lambda_e());
