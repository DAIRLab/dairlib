...
class SafeVelocityController : public drake::systems::LeafSystem<double> {
 public:
  ...
  const drake::systems::InputPort<double>& get_joint_torques_input_port()
      const {
    return this->get_input_port(joint_torques_input_port_);
  }
  const drake::systems::InputPort<double>& get_joint_velocities_input_port()
      const {
    return this->get_input_port(joint_velocities_input_port_);
  }
  const drake::systems::OutputPort<double>& get_joint_torques_output_port()
      const {
