#include "single_state_sampling_c3.h"
#include <iostream>
...
SamplingC3Controller::SamplingC3Controller(
  ...
  all_sample_locations_ = vector<Vector3d>(1, Vector3d::Zero());
  all_sample_costs_ = std::vector<double>(1, -1);
...
drake::systems::EventStatus SamplingC3Controller::ComputePlan(
  ...
  std::vector<VectorXd> x_desired =
      std::vector<VectorXd>(N_ + 1, x_lcs_des);
  ...
  all_sample_costs_ = std::vector<double>(1, -1);
  std::vector<std::shared_ptr<solvers::C3>> c3_objects(1, nullptr);
  std::vector<std::vector<Eigen::VectorXd>> deltas(1, 
                  std::vector<Eigen::VectorXd>(N_,VectorXd::Zero(n_x_ + n_lambda_ + n_u_)));
  std::vector<std::vector<Eigen::VectorXd>> ws(1, 
                  std::vector<Eigen::VectorXd>(N_,VectorXd::Zero(n_x_ + n_lambda_ + n_u_)));

  // Parallelize over computing C3 costs for each sample.
  // Get the candidate state, its LCS representation.
  Eigen::VectorXd test_state = x_lcs_curr;
  solvers::LCS test_system = candidate_lcs_objects.at(0);
  ...
  all_sample_costs_[0] = c3_cost;
  c3_objects.at(0) = test_c3_object;
