...
void LcmDrivenLoop<InputMessageType, SwitchMessageType>::Simulate(
  ...
  const double
      t0 = name_to_input_sub_map_.at(active_channel_).message().utime * 1e-6;
  ...
  // Store robot input
  InputMessageType previous_input =
      name_to_input_sub_map_.at(active_channel_).message();
  ...
  while (time < end_time) {
    // Update active channel name if there are multiple inputs
