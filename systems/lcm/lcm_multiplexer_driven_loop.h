...
/**
 * This class is based on drake::systems::lcm::LcmDrivenLoop, but very slightly
 * modified to utilize a LcmSubscriberMultiplexer. When LcmDrivenLoop is made
 * more abstract, this should become an implementation of that abstract version.
 * 
 ...
 *    message rate.
 */
class LcmMultiplexerDrivenLoop {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(LcmMultiplexerDrivenLoop)

  /**
   * Constructor.
   * @param diagram Const reference to the handler diagram. Its life span must
   * be longer than `this`.
   ...
   */
  LcmMultiplexerDrivenLoop(const drake::systems::Diagram<double>& diagram,
 ...
 private:
  ...
  const LcmSubscriberMultiplexer& driving_mux_;
  ...
  // If true, explicitly calls diagram_.Publish() after every step in the loop.
