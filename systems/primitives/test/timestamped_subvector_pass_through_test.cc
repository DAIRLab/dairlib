...
TEST_F(SubvectorPassThroughTest, VectorThroughPassThroughSystem) {
  ...
  pass_through_->get_input_port().FixValue(context_.get(), *input);
  ...
  EXPECT_NE(nullptr, dynamic_cast<const TimestampedVector<double>*>(
                         output_->get_vector_data(0)));
  auto output_vector = dynamic_cast<const TimestampedVector<double>*>(
      output_->get_vector_data(0));
