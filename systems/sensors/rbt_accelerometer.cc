...
RbtAccelerometer::RbtAccelerometer(const string& name,
  ...
  plant_state_input_port_index_ =
      DeclareInputPort(drake::systems::kVectorValued,
  ...
  output_port_index_ = DeclareVectorOutputPort(
      drake::systems::sensors::AccelerometerOutput<double>(),
      &RbtAccelerometer::CalcAccelerationOutput)
          .get_index();
}

RbtAccelerometer* RbtAccelerometer::AttachRbtAccelerometer(
    const string& name,
    const RigidBodyFrame<double>& frame,
  ...
  // Ensures the input parameters are valid.
  {
    ...
    std::vector<drake::systems::System<double>*> systems =
    ...
    if (!plant_in_builder) {
      throw std::runtime_error("RbtAccelerometer::AttachRbtAccelerometer: ERROR: The "
          "provide DiagramBuilder does not contain the provided "
          "RigidBodyPlant.");
    }
  }

  auto accelerometer = builder->template AddSystem<RbtAccelerometer>(name, frame,
...
void RbtAccelerometer::CalcAccelerationOutput(
