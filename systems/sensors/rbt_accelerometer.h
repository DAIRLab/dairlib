...
///
class RbtAccelerometer : public drake::systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(RbtAccelerometer);

  /// A constructor that initializes an RbtAccelerometer.
  ///
  ...
  ///
  RbtAccelerometer(const std::string& name, const RigidBodyFrame<double>& frame,
      const RigidBodyTree<double>& tree, bool include_gravity = true);

  /// Instantiates and attaches an RbtAccelerometer to a RigidBodyPlant. It
  /// connects the RbtAccelerometer's plant state input port. <b>It does not
  /// connect the plant state derivative input port.</b>
  ///
  /// @param[in] name The name of the sensor. This can be any value.
  ///
  /// @param[in] frame The frame to which the RbtAccelerometer is attached.
  ///
  /// @param[in] plant The plant that the newly instantiated RbtAccelerometer must
  ...
  /// instantiated RbtAccelerometer is added. This must not be `nullptr`.
  ///
  ...
  static RbtAccelerometer* AttachRbtAccelerometer(
      const std::string& name,
      const RigidBodyFrame<double>& frame,
      const drake::systems::RigidBodyPlant<double>& plant,
  ...
  /// sensor is sensing.
  const drake::systems::InputPort<double>& get_plant_state_derivative_input_port()
      const {
 ...
 private:
  ...
  void CalcAccelerationOutput(const drake::systems::Context<double>& context,
      drake::systems::sensors::AccelerometerOutput<double>* output_vector) const;
