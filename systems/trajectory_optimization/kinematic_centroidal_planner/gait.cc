...
drake::trajectories::PiecewisePolynomial<double> Gait::ToTrajectory(
  ...
  while (current_time < end_time) {
    ...
    samples.emplace_back(
        gait_pattern[current_mode].contact_status.cast<double>());

    // Update time based on how much longer the current mode will last
    current_time +=
        (gait_pattern[current_mode].end_phase - current_phase) * period;
...
void Gait::check_valid() const {
  ...
  DRAKE_DEMAND(gait_pattern.back().end_phase == 1);
