...
struct Mode {
  double start_phase;
  double end_phase;
  /// Vector describing which contacts are active
...
/*!
 * @brief Struct for defining a gait consisting of a vector of Modes, and a
 * period for the whole gait.
 */
struct Gait {
  /*!
   ...
   * @param current_time
   * @param end_time
   * @return trajectory where value at time t converted to a bool is a vector of
   * which feet are in stance
   */
  drake::trajectories::PiecewisePolynomial<double> ToTrajectory(
      double current_time, double end_time) const;
  ...
  double period;
  /// Vector of modes. Start phase of the first mode must be
  /// 0, end phase of the last mode
