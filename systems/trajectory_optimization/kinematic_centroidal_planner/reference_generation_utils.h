...
/*!
 * @brief given an initial com and velocity of com emrt w, calculate a com
 * trajectory
 * @param current_com
 * @param vel_ewrt_w velocity of the com ewrt. Given a zoh between time points,
 * must be the same length as time_points
 * @param time_points time values correspeding to velocities, trajectory start
 * at the first, and end at the last, must be the same length as vel_ewrt_w
...
/*!
 * @brief Constructs a trajectory for the generalized positions of a constant
 * joint state and floating base position from com trajectory
 ...
 * @param base_pos_start index where the floating base position starts in
 * generalized state
...
/*!
 * @brief constructs a trajectory for the generalized velocity where the joint
 * velocity is 0 and floating base val from com trajectory
 * @param com_traj
...
/*!
 * @brief given a vector of gaits and time points corresponding to when the
 * gaits are active generate a mode sequence. The transition between gaits can
 * be awkward depending on where the gaits are in phase space during the
 * transition {TODO SRL} make transition clean by attempting to shift phase so
 * mode sequence lines up
 * @param gait_sequence vector of gaits
 * @param time_points vector of time points when each gait is active
 * @return trajectory of mode status start at time_point(0) and ending at
 * time_point.end()-1
 */
...
/*!
 * @brief given a trajectory which describes the mode, and the mass of the robot
 * calculate a nominal grf trajectory where the weight of the robot is
 * distributed over the active contact points
...
/*!
 * @brief Calculate trajectory of world point evaluators from generalized state
 * trajectory. This assumes first order hold between knot points in state
 * trajectories.
 * TODO If we start using more complicated state references with this function
 * sample time more coarsely
 * @param plant
 ...
 * @return trajectory of contact points stacked [contact1_pos, contact1_vel, ...
 * contact_n_pos, contact_n_vel]
 */
