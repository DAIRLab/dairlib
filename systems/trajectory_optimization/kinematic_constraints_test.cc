...
// Matrix3Xd>(KinematicsCache<double> const&, Eigen::MatrixBase<Matrix3Xd>
// const&, int, int);
...
int do_main(int argc, char* argv[]) {
  ...
  parsers::urdf::AddModelInstanceFromUrdfFileToWorld(
      "../../examples/Acrobot/Acrobot.urdf", multibody::joints::kRollPitchYaw,
      &tree);
  ...
  pt << 1, 0, 1;
  bool isXZ = true;
  DirconPositionData<AutoDiffXd> constraintd =
      DirconPositionData<AutoDiffXd>(tree, bodyIdx, pt, isXZ);
  DirconPositionData<AutoDiffXd> constraintd2 =
      DirconPositionData<AutoDiffXd>(tree, bodyIdx, pt2, isXZ);
  DirconPositionData<double> constraint =
      DirconPositionData<double>(tree, bodyIdx, pt, isXZ);
  DirconPositionData<double> constraint2 =
      DirconPositionData<double>(tree, bodyIdx, pt2, isXZ);
  ...
  KinematicsCache<AutoDiffXd> cache =
      tree.doKinematics(q_autodiff, v_autodiff, true);
  ...
  // TODO: add tests for constraint set and dircondynamicconstraint, probably
  // should make these constraints more than just a does-run check (check
  // values)
  ...
  /*
    ...
    KinematicsCache<AutoDiffXd> cache_autodiff =
  tree.get()->doKinematics(q_autodiff,v_autodiff,true);

    AutoDiffVecXd pts =
  tree.get()->transformPoints(cache_autodiff,xB.col(0),idxB[0],0); cout <<
  "***********PTS ***********" << endl; cout << pts << endl; cout <<
  "***********Derivatives***********" << endl; cout <<
  math::autoDiffToGradientMatrix(pts) << endl;
    ...
    //MatrixXd JdotV = tree.get()->transformPointsJacobianDotTimesV(cache,
  xB.col(0), idxB.at(0), 0);
     //VectorXd JdotV =
  tree.get()->transformPointsJacobianDotTimesV<double,Eigen::Block<Eigen::Matrix<double,
  3, -1, 0, 3, -1>, 3, 1, true>>(cache, xB.col(0), idxB.at(0), 0);
  //  auto JdotV = tree.get()->transformPointsJacobianDotTimesV(cache,
  Matrix3Xd(xB.col(0)), idxB.at(0), 0);

    //const Eigen::Map<Matrix3Xd> xB_col0(xB.data(), 3, 1);
    const Eigen::Map<const Eigen::Matrix3Xd> xB_col0(xB.data(), 3, 1);
    //auto JdotV = tree.get()->transformPointsJacobianDotTimesV(cache_autodiff,
  xB_col0, idxB.at(0), 0); AutoDiffVecXd JdotV =
  tree.get()->transformPointsJacobianDotTimesV(cache_autodiff, xB_col0,
  idxB.at(0), 0);
    ...
    // see
  https://github.com/RobotLocomotdion/drake/blob/master/multibody/rigid_body_constraint.cc#L1930~L1959
